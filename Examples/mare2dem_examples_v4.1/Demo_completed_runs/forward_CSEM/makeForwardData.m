...
minRange = 500;  % minimum source to receiver distance to model
...
% And we will also set the 2D tilt (i.e. dip) of the
% transmitter  to the seafloor slope:
dipTx      = interp1(slopePos,slopeAngle,yTx,'previous');
...
%
stCSEM.transmitters = [xTx yTx zTx 90*ones(size(xTx)) dipTx];  % azimuth (heading) is 90 degrees, so it points along y.  
...
if length(xTx) > length(xRx)
    ...
    stCSEM.transmitterName = stCSEM.receiverName;
    stCSEM.receiverName = A;
...
%
dp = zeros(length(stCSEM.frequencies)*length(xRx)*length(xTx)*2,6);
