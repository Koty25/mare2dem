...
! Content:
!       Example of using sfftw_plan_dft_r2c_2d function.
!
!*****************************************************************************

program sp_plan_dft_r2c_2d
  ...
  print *,"Example sp_plan_dft_r2c_2d"
  ...
  call sfftw_plan_dft_r2c_2d(plan_r2c, N1, N2, x_real, x_cmplx, FFTW_ESTIMATE)
...
contains
  ...
  subroutine init(x, N1, N2, H1, H2)
    ...
    forall (k1=1:N1, k2=1:N2)
      x(k1,k2) = 2.0_WP * cos( TWOPI * ( &
    &    real  (moda(H1,k1-1,N1),WP) / real(N1,WP) &
    &    + real(moda(H2,k2-1,N2),WP) / real(N2,WP))) / real((N1*N2),WP)
    end forall
...
end program sp_plan_dft_r2c_2d
