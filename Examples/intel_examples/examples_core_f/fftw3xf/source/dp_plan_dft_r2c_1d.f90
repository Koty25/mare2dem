...
! Content:
!       Example of using dfftw_plan_dft_r2c_1d function.
!
!*****************************************************************************

program dp_plan_dft_r2c_1d
  ...
  print *,"Example dp_plan_dft_r2c_1d"
  ...
  call dfftw_plan_dft_r2c_1d(plan_r2c, N, x_real, x_cmplx, FFTW_ESTIMATE)
...
contains
  ...
  subroutine init(x, N, H)
    ...
    do k = 1, N
      x(k) = 2 * cos( TWOPI * (real(moda(H,k-1,N),WP)/N) ) / N
    end do
    if (mod(H,N)==0) x(:) = x(:) / 2
  end subroutine init
...
end program dp_plan_dft_r2c_1d
