...
! Content:
!       Example of using sfftw_plan_dft_2d function.
!
!*****************************************************************************

program sp_plan_dft_2d
  ...
  call sfftw_plan_dft_2d(fwd, N1, N2, x, x, FFTW_FORWARD, FFTW_ESTIMATE)
  if (0 == fwd) goto 999

  print *,"Create FFTW backward transform plan"
  call sfftw_plan_dft_2d(bwd, N1, N2, x, x, FFTW_BACKWARD, FFTW_ESTIMATE)
  if (0 == bwd) goto 999
...
contains
  ...
  subroutine init(x, N1, N2, H1, H2)
    ...
    &    + real(moda(k2-1,H2, N2), WP)/real(N2,WP) )) / cmplx(N1*N2)
    end forall
  end subroutine init
