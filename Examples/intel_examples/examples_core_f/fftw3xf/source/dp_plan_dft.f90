...
! Content:
!       Example of using dfftw_plan_dft function.
!
!*****************************************************************************
...
program dp_plan_dft
  ...
  ! Working precision is double precision (dfftw_* functions)
  ...
  call dfftw_plan_dft(fwd,4,N(1:4),x,x,FFTW_FORWARD,FFTW_ESTIMATE)
  if (0 == fwd) goto 999

  print *,"Create FFTW plan for backward transform"
  call dfftw_plan_dft(bwd,4,N(1:4),x,x,FFTW_BACKWARD,FFTW_ESTIMATE)
  if (0 == bwd) goto 999
  ...
  subroutine init(x, N, H)
    ...
    forall (k1=1:N(1), k2=1:N(2), k3=1:N(3), k4=1:N(4))
      x(k1,k2,k3,k4) = exp( I_TWOPI * (       &
        real(  moda(k1-1,H(1),N(1)),WP)/N(1)  &
        + real(moda(k2-1,H(2),N(2)),WP)/N(2)  &
        + real(moda(k3-1,H(3),N(3)),WP)/N(3)  &
        + real(moda(k4-1,H(4),N(4)),WP)/N(4) )) / product(N)
    end forall
  end subroutine init
