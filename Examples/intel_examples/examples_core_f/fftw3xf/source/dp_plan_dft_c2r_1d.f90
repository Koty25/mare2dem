...
! Content:
!       Example of using dfftw_plan_dft_c2r_1d function.
!
!*****************************************************************************

program dp_plan_dft_c2r_1d
  ...
  print *,"Example dp_plan_dft_c2r_1d"
  ...
  call dfftw_plan_dft_c2r_1d(plan_c2r, N, x_cmplx, x_real, FFTW_ESTIMATE)
...
contains
  ...
  subroutine init(x, N, H)
    ...
    forall (k=1:N/2+1)
      x(k) = exp( -I_TWOPI * (real(moda(H,k-1,N),WP)/N) ) / N
    end forall
  end subroutine init
...
end program dp_plan_dft_c2r_1d
