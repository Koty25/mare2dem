...
! Content:
!       Example of using dfftw_plan_r2r_1d function.
!
!*****************************************************************************

program dp_plan_r2r_1d
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_REDFT00, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_REDFT10, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_REDFT01, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_REDFT11, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_RODFT00, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_RODFT10, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_RODFT01, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, FFTW_RODFT11, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
  ...
  call dfftw_plan_r2r_1d(r2r, N, x, x, MKL_RODFT00, FFTW_ESTIMATE)
  if (0 == r2r) goto 999
...
contains
  ...
  subroutine init_redft00(x, N, H)
    ...
    do k = 1, N
      x(k) = 2.0 * cos( M_PI * amoda(j,k-1,NL) / (N-1) )
    end do
  ...
  subroutine init_redft10(x, N, H)
    ...
    do k = 1, N
      x(k) = 2.0 * cos( M_PI * amoda(j,2*k-1,2*NL) / (2*N) )
    end do
    if (j==0) x(:) = x(:) / 2.0
    x(:) = x(:) / NL
  ...
  subroutine init_redft01(x, N, H)
    ...
    NL = 2*N ! logical size of transform
    do k = 1, N
      x(k) = 2.0 * cos( M_PI * amoda(2*j+1,k-1,2*NL) / (2*N) )
    end do
    x(:) = x(:) / NL
  ...
  subroutine init_redft11(x, N, H)
    ...
    NL = 2*N ! logical size of transform
    do k = 1, N
      x(k) = 2.0 * cos( M_PI * amoda(2*j+1,2*k-1,4*NL) / (4*N) )
    end do
    x(:) = x(:) / NL
  ...
  subroutine init_rodft00(x, N, H)
    ...
    NL = 2*(N+1) ! logical size of transform
    do k = 1, N
      x(k) = 2.0 * sin( M_PI * amoda(j+1,k,NL) / (N+1) )
    end do
    x(:) = x(:) / NL
  ...
  subroutine init_rodft10(x, N, H)
    ...
    NL = 2*N ! logical size of transform
    do k = 1, N
      x(k) = 2.0 * sin( M_PI * amoda(j+1,2*k-1,2*NL) / (2*N) )
    end do
    if (j==N-1) x(:) = x(:) / 2.0
    x(:) = x(:) / NL
  ...
  subroutine init_rodft01(x, N, H)
    ...
    NL = 2*N ! logical size of transform
    do k = 1, N
      x(k) = 2.0 * sin( M_PI * amoda(2*j+1,k,2*NL) / (2*N) )
    end do
    x(:) = x(:) / NL
  ...
  subroutine init_rodft11(x, N, H)
    ...
    NL = 2*N ! logical size of transform
    do k = 1, N
      x(k) = 2.0 * sin( M_PI * amoda(2*j+1,2*k-1,4*NL) / (4*N) )
    end do
    x(:) = x(:) / NL
