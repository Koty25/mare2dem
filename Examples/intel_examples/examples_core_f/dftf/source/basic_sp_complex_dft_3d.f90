...
program basic_sp_complex_dft_3d
  ...
  print *,"Example basic_sp_complex_dft_3d"
  ...
  status = DftiComputeBackward(hand, x(:,1,1))
  if (0 /= status) goto 999
...
contains
  ...
  subroutine init(x, N1, N2, N3, H1, H2, H3)
    ...
    &    moda(  k1-1,H1, N1)/real(N1,WP) &
    &    + moda(k2-1,H2, N2)/real(N2,WP) &
    &    + moda(k3-1,H3, N3)/real(N3,WP) )) / cmplx(N1*N2*N3)
    end forall
  end subroutine init
...
end program basic_sp_complex_dft_3d
