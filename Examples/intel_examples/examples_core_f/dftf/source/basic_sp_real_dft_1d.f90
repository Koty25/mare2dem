...
program basic_sp_real_dft_1d
  ...
  print *,"Example basic_sp_real_dft_1d"
  print *,"Forward-Backward single-precision real-to-complex",               &
...
contains
  ...
  subroutine init_r(x, N, H)
    ...
    forall (k=1:N)
      x(k) = 2.0_WP * cos( TWOPI * moda(H,k-1,N) / real(N,WP)) / real(N,WP)
    end forall
  ...
  subroutine init_c(x, N, H)
    ...
    forall (k=1:N/2+1)
      x(k) = exp( -ITWOPI * moda(H,k-1,N) / cmplx(N)) / cmplx(N)
    end forall
...
end program basic_sp_real_dft_1d
