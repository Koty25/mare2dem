...
!*******************************************************************************/
      ...
      PROGRAM vsldconvolution_2d_direct
      USE MKL_VSL_TYPE
      USE MKL_VSL
      IMPLICIT NONE
C
...
C
      ...
      REAL*8 x(3,2),y(4,3),z(6,4),e(6,4)
      DATA x/1,1,1, 1,1,1/
...
C
      ...
      ENDIF
C
C  Execute task (Calculate 2 dimension convolution of two arrays)
C
...
C
      DO j=1,zshape(2)
        DO i=1,zshape(1)
            IF (dabs(z(i,j)-e(i,j)) .GT. e(i,j)*1d-10) THEN
                PRINT *, "ERROR:"
                PRINT *, "        i :", i
                PRINT *, "        j :", j
                PRINT *, "    z(i,j):", z(i,j)
