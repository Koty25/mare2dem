...
!******************************************************************************/
      ...
      real(kind=8),parameter :: EPSILON = 1e-6
     ...
     &             0.1,  0.1,  0.1,  1.0 /
      ...
      real(kind=8) dummy_x(DIM,1)
      real(kind=8) cp_cov(DIM,DIM)
      real(kind=8) pcov(PART_DIM,PART_DIM)
      real(kind=8) th_pcov(PART_DIM,PART_DIM)
      integer task_method
...
!     ***** Edit task parameters for partial covariance matrix computation *****
      errcode = vsldsseditpartialcovcor( task, pcov_index, cov,         &
...
!     ***** Compute partial covariance matrix using FAST method *****
      status = vsldsscompute( task, task_params, task_method )
      call CheckVslError( errcode )
