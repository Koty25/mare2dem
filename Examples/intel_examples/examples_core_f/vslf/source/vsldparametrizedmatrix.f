...
!******************************************************************************/
      ...
      integer(kind=8),parameter :: LLWORK = 25*DIM

      real(kind=8),parameter :: TEST_THRESHOLD = -1e-6

!     Distorted correlation matrix
      real(kind=8) cor(DIM*DIM)
     ...
     &            0.7, 0.29,  1.0 /
      ...
      real(kind=8) dummy_x(1)
      real(kind=8) pcor(DIM*DIM)
      real(kind=8) copy_cor(DIM*DIM)
      integer i, k1, k2
...
!     ***** Following variables are used in DSYEV routine which finds
!           eigenvalues of simmetric matrix *****
      real(kind=8) eigenvals(DIM), work(LLWORK)
...
!     ***** Create Summary Statistics task *****
      errcode = vsldssnewtask( task, p, dummy_n, dummy_x_storage,       &
     &                         dummy_x )
      call CheckVslError( errcode )

!     ***** Edit task parameters for parameterization of correlation *****
      errcode = vsldsseditcorparameterization( task, cor,               &
...
!     ***** Compute eigenvalues of distorted correlation matrix *****
      ...
      uplo = 'U'
      jobz = 'N'
      call dsyev( jobz, uplo, p, copy_cor, p, eigenvals, work, lwork,   &
     &            info )
