...
!===============================================================================
      ...
      SUBROUTINE SDDCON(h,inch,x,incx,y,incy,nh,nx,iy0,ny,id)
      USE MKL_VSL_TYPE
      USE MKL_VSL
      IMPLICIT NONE
!
...
!
      start(1) = iy0
      decimation(1) = id
      double_precision = VSL_CONV_PRECISION_DOUBLE
      ...
      status = vslconvsetinternalprecision(task,double_precision)
      status = vslsconvexec1d(task,h,inch,x,incx,y,incy)
      error  = vslconvdeletetask(task)
!
      IF (status .NE. VSL_STATUS_OK) THEN
      PRINT *, 'ERROR: sddcon(): bad status=',status
      STOP 1
      END IF
!
      IF (error .NE. 0) THEN
      PRINT *, 'ERROR: sddcon(): failed to destroy the task descriptor'
      STOP 1
      END IF
!
      RETURN
