...
!  Content:
!  Fortran-90 single precision example of solving 2D Helmholtz problem in a
!  rectangular domain using Intel(R) Math Kernel Library (Intel(R) MKL)
!  Poisson Library
!
!*******************************************************************************

program Helmholtz_2D_single_precision
...
do iy=1,ny+1
   do ix=1,nx+1
      ...
      u(ix,iy)=1.0E0*cx*cy
      f(ix,iy)=((8.0E0*pi**2)+q)*u(ix,iy)+q
...
! Setting the values of the boundary function G(x,y) that is equal to the TRUE
! solution in the mesh points laying on Dirichlet boundaries
do iy = 1,ny+1
   bd_ax(iy) = 1.0E0
   bd_bx(iy) = 1.0E0
enddo
...
print *, ' Single precision 2D Helmholtz example has successfully PASSED'
print *, ' through all steps of computation!'
stop 0
! Failure message to print if something went wrong
999 print *, 'Single precision 2D Helmholtz example FAILED to compute the solution...'
    ! Free Intel MKL memory if any was allocated
    call mkl_free_buffers
    stop 1
