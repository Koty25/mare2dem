...
!  Content:
!  Fortran-90 double precision example of solving 3D Helmholtz problem in a
!  parallelepiped domain using Intel(R) Math Kernel Library (Intel(R) MKL)
!  Poisson Library
!
!*******************************************************************************

program Helmholtz_3D_double_precision
...
do iz=1,nz+1
   do iy=1,ny+1
      do ix=1,nx+1
         ...
         f(ix,iy,iz)=((12.0D0*pi**2)+q)*u(ix,iy,iz)+q
...
print *, ' Double precision 3D Helmholtz example has successfully PASSED'
print *, ' through all steps of computation!'
stop 0
! Failure message to print if something went wrong
999 print *, 'Double precision 3D Helmholtz example FAILED to compute the solution...'
    ! Free Intel MKL memory if any was allocated
    call mkl_free_buffers
    stop 1
