...
!  Content:
!  Fortran-90 single precision example of solving 3D Helmholtz problem in a
!  parallelepiped domain using Intel(R) Math Kernel Library (Intel(R) MKL)
!  Poisson Library
!
!*******************************************************************************

program Helmholtz_3D_single_precision
...
print *, ' **********************************************'
print *, ''
print *, ' This example gives the solution of 3D Helmholtz problem'
print *, ' with the equation -u_xx-u_yy-u_zz+u=f(x,y,z), 0<x<1, 0<y<1, 0<z<1,'
print *, ' f(x,y,z)=(12*pi*pi+1)*sin(2*pi*x)*sin(2*pi*y)*sin(2*pi*z)+1,'
...
print *, ' The approximation error should be of order 0.5E+0,',&
         ' if everything is OK'
print *, ' -----------------------------------------------------------------------'
...
do iz=1,nz+1
   do iy=1,ny+1
      do ix=1,nx+1
         ...
         f(ix,iy,iz)=((12.0E0*pi**2)+q)*u(ix,iy,iz)+q
...
call s_init_Helmholtz_3D(ax, bx, ay, by, az, bz, nx, ny, nz, BCtype, q, &
                                                                     ipar, spar, stat)
...
print *, ' Single precision 3D Helmholtz example has successfully PASSED'
print *, ' through all steps of computation!'
stop 0
! Failure message to print if something went wrong
999 print *, 'Single precision 3D Helmholtz example FAILED to compute the solution...'
    ! Free Intel MKL memory if any was allocated
    call mkl_free_buffers
    stop 1
