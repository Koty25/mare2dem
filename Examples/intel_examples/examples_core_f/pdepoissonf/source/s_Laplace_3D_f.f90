...
!  Content:
!  Fortran-90 single precision example of solving 3D Laplace problem in a
!  parallelepiped domain using Intel(R) Math Kernel Library (Intel(R) MKL)
!  Poisson Library
!
!*******************************************************************************

program Laplace_3D_single_precision
...
integer ipar(128)
real ax, bx, ay, by, az, bz, lx, ly, lz, hx, hy, hz, xi, yi, zi, c1
...
print *, ' The approximation error should be of order 0.2E-1, if everything is OK'
print *, ' -----------------------------------------------------------------------'
...
do iz=1,nz+1
   do iy=1,ny+1
      do ix=1,nx+1
         ...
         u(ix,iy,iz)=xi**4+yi**4+zi**4-3.0E0*((xi**2)*(yi**2)+(xi**2)*(zi**2)+&
                                                                      (yi**2)*(zi**2))
         f(ix,iy,iz)=0.0E0
      enddo
   enddo
...
do iy = 1,ny+1
   do iz = 1,nz+1
      yi=hy*(iy-1)/ly
      zi=hz*(iz-1)/lz
      bd_ax(iy,iz) = yi**4+zi**4-3.0E0*(yi**2)*(zi**2)
      bd_bx(iy,iz) = 1.0E0+yi**4+zi**4-3.0E0*(yi**2+zi**2+(yi**2)*(zi**2))
   enddo
enddo
do ix = 1,nx+1
   do iz = 1,nz+1
      xi=hx*(ix-1)/lx
      zi=hz*(iz-1)/lz
      bd_ay(ix,iz) = xi**4+zi**4-3.0E0*(xi**2)*(zi**2)
      bd_by(ix,iz) = xi**4+1.0E0+zi**4-3.0E0*(xi**2+(xi**2)*(zi**2)+zi**2)
   enddo
enddo
do ix = 1,nx+1
   do iy = 1,ny+1
      xi=hx*(ix-1)/lx
      yi=hy*(iy-1)/ly
      bd_az(ix,iy) = xi**4+yi**4-3.0E0*(xi**2)*(yi**2)
      bd_bz(ix,iy) = xi**4+yi**4+1.0E0-3.0E0*((xi**2)*(yi**2)+xi**2+yi**2)
   enddo
enddo
...
print *, ' Single precision 3D Laplace example has successfully PASSED'
print *, ' through all steps of computation!'
stop 0
! Failure message to print if something went wrong
999 print *, 'Single precision 3D Laplace example FAILED to compute the solution...'
    ! Free Intel MKL memory if any was allocated
    call mkl_free_buffers
    stop 1
