...
!  Content:
!  Fortran-90 double precision example of solving 2D Laplace problem in a
!  rectangular domain using Intel(R) Math Kernel Library (Intel(R) MKL)
!  Poisson Library
!
!*******************************************************************************

program Laplace_2D_double_precision
...
integer ipar(128)
double precision ax, bx, ay, by, lx, ly, hx, hy, xi, yi, c1
...
print *, ' The approximation error should be of order 0.5D-2, if everything is OK'
print *, ' -----------------------------------------------------------------------'
...
do iy=1,ny+1
   do ix=1,nx+1
      ...
      u(ix,iy)=xi**4+yi**4-6.0D0*(yi**2)*(xi**2)
      f(ix,iy)=0.0D0
   enddo
enddo
...
do iy = 1,ny+1
   yi=hy*(iy-1)/ly
   bd_ax(iy) = yi**4
   bd_bx(iy) = 1.0D0-6.0D0*yi**2+yi**4
enddo
do ix = 1,nx+1
   xi=hx*(ix-1)/lx
   bd_ay(ix) = xi**4
   bd_by(ix) = 1.0D0-6.0D0*xi**2+xi**4
enddo
...
print *, ' Double precision 2D Laplace example has successfully PASSED'
print *, ' through all steps of computation!'
stop 0
! Failure message to print if something went wrong
999 print *, 'Double precision 2D Laplace example FAILED to compute the solution...'
    ! Free Intel MKL memory if any was allocated
    call mkl_free_buffers
    stop 1
