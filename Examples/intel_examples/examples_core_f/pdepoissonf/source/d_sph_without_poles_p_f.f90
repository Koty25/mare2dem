...
!  Content:
!  Fortran-90 double precision example of solving Helmholtz problem on a whole
!  sphere excluding poles using Intel(R) Math Kernel Library (Intel(R) MKL)
!  Poisson Library
!
!*******************************************************************************

program d_sph_without_poles_p_f
...
do it=1,nt+1
   do ip=1,np+1
      ...
      u(ip,it)=ct
      f(ip,it)=q*ct+2.0D0-(dcos(theta_i)/dsin(theta_i))*(at+bt-2*theta_i)
   enddo
enddo
...
print *, ' Double precision Helmholtz example on a sphere without poles has'
print *, ' successfully PASSED through all steps of computation!'
stop 0
! Failure message to print if something went wrong
999 print *, 'Double precision Helmholtz example on a sphere without poles has'
    print *, 'FAILED to compute the solution...'
    ! Free Intel MKL memory if any was allocated
    call mkl_free_buffers
    stop 1
