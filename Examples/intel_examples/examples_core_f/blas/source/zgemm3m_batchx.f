...
!  Content:
!      Z G E M M 3 M _ B A T C H  Example Program Text
!*******************************************************************************

      program   ZGEMM3M_BATCH_MAIN
*
     ...
     $                 rmaxc=5, cmaxc=5)
      ...
      complex*16       a1, b1
      complex*16       alpha(maxgc), beta(maxgc)
      complex*16       a(rmaxa,cmaxa,maxmat)
      complex*16       b(rmaxb,cmaxb,maxmat)
      complex*16       c(rmaxc,cmaxc,maxmat)
      character*1      transa(maxgc), transb(maxgc)
      integer(KIND=C_SIZE_T) a_array(maxmat), b_array(maxmat), 
     $                       c_array(maxmat)
 
*       External Subroutines
      external         ZGEMM3M_BATCH, PrintArrayZ
*
*      Executable Statements
*
      print*
      print*,'   Z G E M M 3 M _ B A T C H  EXAMPLE PROGRAM'
*
*      Read input data
      read*
      read*, grpcount
...
*       Print input data
      ...
      do ig = 1, grpcount
        ...
        do is = 1, grpsize(ig)
          if ((transa(ig).eq.'N').or.(transa(ig).eq.'n')) then
            call PrintArrayZ(0,0,m(ig),k(ig),a(:,:,matidx),lda(ig),'A')
          else
            call PrintArrayZ(0,0,k(ig),m(ig),a(:,:,matidx),lda(ig),'A')
          end if
          if ((transb(ig).eq.'N').or.(transb(ig).eq.'n')) then
            call PrintArrayZ(0,0,k(ig),n(ig),b(:,:,matidx),ldb(ig),'B')
          else
            call PrintArrayZ(0,0,n(ig),k(ig),b(:,:,matidx),ldb(ig),'B')
          end if
          call PrintArrayZ(0,0,m(ig),n(ig),c(:,:,matidx),ldc(ig),'C')
          matidx = matidx + 1
        end do
      end do
*
*      Call ZGEMM3M_BATCH subroutine
      call ZGEMM3M_BATCH(transa,transb,m,n,k,alpha,a_array,lda,b_array,
...
*
      ...
      do ig = 1, grpcount
        do is = 1, grpsize(ig)
          call PrintArrayZ(1,0,m(ig),n(ig),c(:,:,matidx),ldc(ig),'C')
          matidx = matidx + 1
        end do
      end do
