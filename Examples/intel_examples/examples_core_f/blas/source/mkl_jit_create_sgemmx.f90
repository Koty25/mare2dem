...
!  Content:
!      MKL_JIT_CREATE_SGEMM  Example Program Text
!*******************************************************************************

include "mkl_blas.f90"

      program   MKL_JIT_CREATE_SGEMM_MAIN
!
      ...
      real(KIND=c_float)       a(rmaxa,cmaxa), b(rmaxb,cmaxb), c(rmaxc,cmaxc)
      TYPE(C_FUNPTR)           c_kernel

      PROCEDURE(sgemm_jit_kernel_t),POINTER :: f_kernel
...
!
      print*
      print*,'   MKL_JIT_CREATE_SGEMM EXAMPLE PROGRAM'
...
!
!      Call MKL_JIT_CREATE_DGEMM subroutine
      ...
      c_kernel = mkl_jit_get_sgemm_ptr(jitter)
