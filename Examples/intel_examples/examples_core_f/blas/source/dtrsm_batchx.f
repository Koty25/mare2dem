...
!  Content:
!      D T R S M _ B A T C H  Example Program Text
!*******************************************************************************

      program   DTRSM_BATCH_MAIN
*
      ...
      double precision alpha(maxgc)
      double precision a(rmaxa,cmaxa,maxmat)
      double precision b(rmaxb,cmaxb,maxmat)
...
*       External Subroutines
      external         DTRSM_BATCH, PrintArrayD
*
*      Executable Statements
*
      print*
      print*,'   D T R S M _ B A T C H  EXAMPLE PROGRAM'
*
*      Read input data
      read*
      read*, grpcount
...
*       Print input data
      ...
      do ig = 1, grpcount
        ...
        do is = 1, grpsize(ig)
          if ((side(ig).eq.'L').or.(side(ig).eq.'l')) then
            if ((uplo(ig).eq.'U').or.(uplo(ig).eq.'u')) then
             call PrintArrayD(0,1,m(ig),m(ig),a(:,:,matidx),lda(ig),'A')
            else
             call PrintArrayD(0,-1,m(ig),m(ig),a(:,:,matidx),
     $                        lda(ig),'A')
            end if
          else
            if ((uplo(ig).eq.'U').or.(uplo(ig).eq.'u')) then
              call PrintArrayD(0,1,n(ig),n(ig),a(:,:,matidx),
     $                         lda(ig),'A')
            else
              call PrintArrayD(0,-1,n(ig),n(ig),a(:,:,matidx),
     $                         lda(ig),'A')
          ...
          call PrintArrayD(0,0,m(ig),n(ig),b(:,:,matidx),ldb(ig),'B')
          matidx = matidx + 1
        end do
      end do
*
*      Call DTRSM_BATCH subroutine
      call DTRSM_BATCH(side,uplo,transa,diag,m,n,alpha,a_array,lda,
...
*
      ...
      do ig = 1, grpcount
        do is = 1, grpsize(ig)
          call PrintArrayD(1,0,m(ig),n(ig),b(:,:,matidx),ldb(ig),'B')
          matidx = matidx + 1
        end do
      end do
