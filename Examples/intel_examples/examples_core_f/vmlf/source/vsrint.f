...
!*******************************************************************************
      ...
      call VSRINT(vec_len,fA,fBha0)
      call VSRINTI(vec_len,fA_I,inca,fBha0_I,incb)

      mode=VML_EP
      call VMSRINT(vec_len,fA,fBep1,mode)
      call VMSRINTI(vec_len,fA_I,inca,fBep1_I,incb,mode)

      tmode=VML_EP
      tmode=VMLSETMODE(tmode)
      call VSRINT(vec_len,fA,fBep2)
      call VSRINTI(vec_len,fA_I,inca,fBep2_I,incb)

      mode=VML_LA
      call VMSRINT(vec_len,fA,fBla1,mode)
      call VMSRINTI(vec_len,fA_I,inca,fBla1_I,incb,mode)

      tmode=VML_LA
      tmode=VMLSETMODE(tmode)
      call VSRINT(vec_len,fA,fBla2)
      call VSRINTI(vec_len,fA_I,inca,fBla2_I,incb)

      mode=VML_HA
      call VMSRINT(vec_len,fA,fBha1,mode)
      call VMSRINTI(vec_len,fA_I,inca,fBha1_I,incb,mode)

      tmode=VML_HA
      tmode=VMLSETMODE(tmode)
      call VSRINT(vec_len,fA,fBha2)
      call VSRINTI(vec_len,fA_I,inca,fBha2_I,incb)

      do i=1,10
          if(fBha0(i) .ne. fBha1(i)) then
              print *,"Error! Difference between VSRINT and"
              print *," VMSRINT in VML_HA mode detected"
              stop 1
          endif
          if(fBha0_I((i-1)*incb+1) .ne. fBha1_I((i-1)*incb+1)) then
              print *,"Error! Difference between VSRINTI and"
              print *," VMSRINTI in VML_HA mode detected"
              stop 1
          endif
          ...
          if(fBla1(i) .ne. fBla2(i)) then
              print *,"Error! Difference between VSRINT and"
              print *," VMSRINT in VML_LA mode detected"
              stop 1
          endif
          if(fBla1_I((i-1)*incb+1) .ne. fBla2_I((i-1)*incb+1)) then
              print *,"Error! Difference between VSRINTI and"
              print *," VMSRINTI in VML_LA mode detected"
              stop 1
          endif
          if(fBep1(i) .ne. fBep2(i)) then
              print *,"Error! Difference between VSRINT and"
              print *," VMSRINT in VML_EP mode detected"
              stop 1
          endif
          if(fBep1_I((i-1)*incb+1) .ne. fBep2_I((i-1)*incb+1)) then
              print *,"Error! Difference between VSRINTI and"
              print *," VMSRINTI in VML_EP mode detected"
              stop 1
          endif
      end do
     ...
     &        "========================"
      ...
      print *,""
      print *,"vsRintI test/example program"
      print *,"           Argument                     vsRint"
      print *,"======================================================", &
     &        "========================"
      do i=1,10
