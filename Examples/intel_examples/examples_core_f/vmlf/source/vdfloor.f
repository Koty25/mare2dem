...
!*******************************************************************************
      ...
      call VDFLOOR(vec_len,dA,dBha0)
      call VDFLOORI(vec_len,dA_I,inca,dBha0_I,incb)

      mode=VML_EP
      call VMDFLOOR(vec_len,dA,dBep1,mode)
      call VMDFLOORI(vec_len,dA_I,inca,dBep1_I,incb,mode)

      tmode=VML_EP
      tmode=VMLSETMODE(tmode)
      call VDFLOOR(vec_len,dA,dBep2)
      call VDFLOORI(vec_len,dA_I,inca,dBep2_I,incb)

      mode=VML_LA
      call VMDFLOOR(vec_len,dA,dBla1,mode)
      call VMDFLOORI(vec_len,dA_I,inca,dBla1_I,incb,mode)

      tmode=VML_LA
      tmode=VMLSETMODE(tmode)
      call VDFLOOR(vec_len,dA,dBla2)
      call VDFLOORI(vec_len,dA_I,inca,dBla2_I,incb)

      mode=VML_HA
      call VMDFLOOR(vec_len,dA,dBha1,mode)
      call VMDFLOORI(vec_len,dA_I,inca,dBha1_I,incb,mode)

      tmode=VML_HA
      tmode=VMLSETMODE(tmode)
      call VDFLOOR(vec_len,dA,dBha2)
      call VDFLOORI(vec_len,dA_I,inca,dBha2_I,incb)

      do i=1,10
          if(dBha0(i) .ne. dBha1(i)) then
              print *,"Error! Difference between VDFLOOR and"
              print *," VMDFLOOR in VML_HA mode detected"
              stop 1
          endif
          if(dBha0_I((i-1)*incb+1) .ne. dBha1_I((i-1)*incb+1)) then
              print *,"Error! Difference between VDFLOORI and"
              print *," VMDFLOORI in VML_HA mode detected"
              stop 1
          endif
          ...
          if(dBla1(i) .ne. dBla2(i)) then
              print *,"Error! Difference between VDFLOOR and"
              print *," VMDFLOOR in VML_LA mode detected"
              stop 1
          endif
          if(dBla1_I((i-1)*incb+1) .ne. dBla2_I((i-1)*incb+1)) then
              print *,"Error! Difference between VDFLOORI and"
              print *," VMDFLOORI in VML_LA mode detected"
              stop 1
          endif
          if(dBep1(i) .ne. dBep2(i)) then
              print *,"Error! Difference between VDFLOOR and"
              print *," VMDFLOOR in VML_EP mode detected"
              stop 1
          endif
          if(dBep1_I((i-1)*incb+1) .ne. dBep2_I((i-1)*incb+1)) then
              print *,"Error! Difference between VDFLOORI and"
              print *," VMDFLOORI in VML_EP mode detected"
              stop 1
          endif
      end do
     ...
     &        "========================"
      ...
      print *,""
      print *,"vdFloorI test/example program"
      print *,"           Argument                     vdFloor"
      print *,"======================================================", &
     &        "========================"
      do i=1,10
