...
!*******************************************************************************
      ...
      call VSEXPI(vec_len,fA_I,inca,fBha0_I,incb)

      mode=VML_EP
      call VMSEXP(vec_len,fA,fBep1,mode)
      call VMSEXPI(vec_len,fA_I,inca,fBep1_I,incb,mode)

      tmode=VML_EP
      tmode=VMLSETMODE(tmode)
      call VSEXP(vec_len,fA,fBep2)
      call VSEXPI(vec_len,fA_I,inca,fBep2_I,incb)

      mode=VML_LA
      call VMSEXP(vec_len,fA,fBla1,mode)
      call VMSEXPI(vec_len,fA_I,inca,fBla1_I,incb,mode)

      tmode=VML_LA
      tmode=VMLSETMODE(tmode)
      call VSEXP(vec_len,fA,fBla2)
      call VSEXPI(vec_len,fA_I,inca,fBla2_I,incb)

      mode=VML_HA
      call VMSEXP(vec_len,fA,fBha1,mode)
      call VMSEXPI(vec_len,fA_I,inca,fBha1_I,incb,mode)

      tmode=VML_HA
      tmode=VMLSETMODE(tmode)
      call VSEXP(vec_len,fA,fBha2)
      call VSEXPI(vec_len,fA_I,inca,fBha2_I,incb)
      ...
      do i=1,10
          ...
          if(fBha0_I((i-1)*incb+1) .ne. fBha1_I((i-1)*incb+1)) then
              print *,"Error! Difference between VSEXPI and"
              print *," VMSEXPI in VML_HA mode detected"
              stop 1
          endif
          ...
          if(fBla1_I((i-1)*incb+1) .ne. fBla2_I((i-1)*incb+1)) then
              print *,"Error! Difference between VSEXPI and"
              print *," VMSEXPI in VML_LA mode detected"
              stop 1
          endif
          ...
          if(fBep1_I((i-1)*incb+1) .ne. fBep2_I((i-1)*incb+1)) then
              print *,"Error! Difference between VSEXPI and"
              print *," VMSEXPI in VML_EP mode detected"
              stop 1
          endif
      end do
     ...
     &        "========================"
      ...
      print *,""
      print *,"vsExpI test/example program"
      print *,"           Argument                     vsExp"
      print *,"======================================================", &
     &        "========================"
      do i=1,10
