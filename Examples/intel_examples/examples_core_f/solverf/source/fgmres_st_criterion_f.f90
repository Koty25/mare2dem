...
! Request to the DFGMRES_GET routine to put the solution into RHS[N] via IPAR(13)
            IPAR(13) = 1
! Get the current FGMRES solution in the vector RHS(N)
            CALL DFGMRES_GET(N, COMPUTED_SOLUTION, RHS, RCI_REQUEST, &
     &  IPAR, DPAR, TMP, ITERCOUNT)
! Compute the current true residual via Intel MKL (Sparse) BLAS routines
        info = MKL_SPARSE_D_MV(SPARSE_OPERATION_NON_TRANSPOSE,alpha,csrA,&
&                              descrA,RHS,beta,RESIDUAL)
...
! into vector COMPUTED_SOLUTION(N) via IPAR(13)
!---------------------------------------------------------------------------
3       IPAR(13) = 0
...
999     WRITE( *,'(A,A,I5)') 'This example FAILED as the solver has', &
     &  ' returned the ERROR code', RCI_REQUEST
        ...
        CALL MKL_FREE_BUFFERS
        STOP 1
      END PROGRAM FGMRES_ST_CRITERION_F
