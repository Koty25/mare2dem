...
!*******************************************************************************
      ...
      REAL(4) :: freq,left,right
      REAL(4) :: spline_val(N),left_val(N),right_val(N)
      REAL(4) :: left_der(N),right_der(N)
      INTEGER :: i,j,errnums
      INTEGER(4) :: errcode
     ...
     &    ic_type, ic, scoeff, scoeffhint )
      ...
      errcode = sCheckSubbBreakPoints( nx, x, ny, y,                     &
     &    scoeff, spline_val )
      IF ( errcode < 0 ) errnums = errnums+1

      !***** Check spline values and 1st derivatives consistency
      !      in Subbotin spline knots *****
      errcode = sCheckSubbQNodes( nx, x, nic, ic, ny,                    &
     &    scoeff, left_val, right_val )
      IF ( errcode < 0 ) errnums = errnums+1

      errcode = sCheckQuadSubb1stDerConsistency( nic, x, ic, ny,         &
     &     scoeff, left_der, right_der )
      IF ( errcode < 0 ) errnums = errnums+1

      !***** Check boundary conditions *****
      errcode = sCheckQuadBC( nx, ic, ny,  scoeff, bc_type, bc )
      IF ( errcode < 0 ) errnums = errnums+1
