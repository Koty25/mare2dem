...
!******************************************************************************/
     ...
     &      OPTIONAL                    :: libparams
      ...
      REAL(4),PARAMETER :: LLIM_X      =  0.0d0
      ! right limit of interpolation interval
      REAL(4),PARAMETER :: RLIM_X      =  3.0d0
      ! left  limit of integration interval
      REAL(4),PARAMETER :: LLIM_INTEGR = -0.5d0
      ! right limit of integration interval
      REAL(4),PARAMETER :: RLIM_INTEGR =  3.5d0
      REAL(4),PARAMETER :: FFREQ       =  0.75d0
      ...
      freq = FFREQ
      errcode = sSinDataNotUniformGrid( y(1:N), x, freq, nx )
      CALL CheckDfError(errcode)
      !***** Generate function y2 = sin(2 * Pi * freq * x) *****
      freq = FFREQ*2.0
      errcode = sSinDataNotUniformGrid( y(N+1:2*N), x, freq, nx )
      CALL CheckDfError(errcode)
     ...
     &     ic_type, scoeff=scoeff, scoeffhint=scoeffhint )
      ...
      errcode = dfsintegrateex1d( task, DF_METHOD_PP, nlim, llim,        &
