...
!*******************************************************************************
      ...
      REAL(4),PARAMETER :: RIGHT_LIMIT = 3.0

      REAL(4),PARAMETER :: FFREQ = 0.5
      ...
      REAL(4),TARGET  :: x(N)
      REAL(4),POINTER :: x_ptr(:)
      ! function values
      REAL(4) :: y(NNY*N)
      ! array of internal conditions
      REAL(4) :: ic(NNIC)
      ! array of boundary conditions
      REAL(4),TARGET  :: bc(NNBC)
      REAL(4),POINTER :: bc_ptr(:)
      ...
      x_ptr => x(2:)
      errcode = sSinDer2DataNotUniformGrid( ic, x_ptr, freq , nic )
      CALL CheckDfError(errcode)

      !***** Generate boundary conditions *****
      x_ptr => x(1:)
      bc_ptr => bc(1:)
      errcode = sSinDer2DataNotUniformGrid( bc_ptr, x_ptr, freq , 1 )
      CALL CheckDfError(errcode)
      x_ptr => x(nx:)
      bc_ptr => bc(2:)
      errcode = sSinDer2DataNotUniformGrid( bc_ptr, x_ptr, freq , 1 )
      ...
      errcode = dfsnewtask1d( task, nx, x, xhint, ny, y )
      CALL CheckDfError(errcode)
     ...
     &    left_der2, right_der2 )
      ...
      DO j = 1, nic
        IF ( ABS( ic(j) - left_der2(j) ) > EPSILON_SINGLE )              &
     &       errnums = errnums+1
      END DO
