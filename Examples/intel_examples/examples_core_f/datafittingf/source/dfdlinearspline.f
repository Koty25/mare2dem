...
!*******************************************************************************
      ...
      REAL(8),PARAMETER :: LEFT_LLIM  = -1.5d0
      REAL(8),PARAMETER :: RIGHT_LLIM = -0.5d0
      REAL(8),PARAMETER :: LEFT_RLIM  =  0.5d0
      REAL(8),PARAMETER :: RIGHT_RLIM =  1.5d0
      REAL(8),PARAMETER :: FFREQ      =  1.3d0
      ...
      DO i = 0, ny-1
        freq = (i + 1) * FFREQ
        y_ptr => y(i*nx+1:)
        errcode = dSinDataNotUniformGrid( y_ptr, x, freq, nx )
        CALL CheckDfError(errcode)
      END DO
     ...
     &    llimhint, rlim, rlimhint, r=r, rhint=rhint )
      ...
      errcode = dCheckLinSplineBreakPoints( nx, x, ny, y, scoeff )
     ...
     &                  scoeff_ptr(sorder*j + 2)
      ...
      errcode = dUniformData(llim_full, llim(1), llim(2), nlim)
      CALL CheckDfError(errcode)
      errcode = dUniformData(rlim_full, rlim(1), rlim(2), nlim)
      CALL CheckDfError(errcode)
      DO i = 0, ny-1
