...
!*******************************************************************************
      ...
      INTEGER,PARAMETER :: LEFT_LIMIT  = 1.0
      ! right limit of interpolation interval
      INTEGER,PARAMETER :: RIGHT_LIMIT = 2.0

      INTEGER,PARAMETER :: FFREQ = 1.7
      ...
      errcode = dfsEditPPSpline1D( task, sorder, stype )
      CALL CheckDfError(errcode)
     ...
     &    site, sitehint, ndorder, derorder, r=r, rhint=rhint )
      ...
      DO j = 1, nsite
        IF ( ABS( r(j) - y(j) ) > EPSILON_SINGLE ) errnums = errnums+1
      END DO
