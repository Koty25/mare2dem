...
!*****************************************************************************
      ...
      PROGRAM REAL_3D_SINGLE_EX6

      INCLUDE 'fftw_f77.i'
      INCLUDE 'mkl_fftw_examples.fi'

      INTEGER N,M,L,NTHREADS,STATUS,OUT_N,I,J,K
      PARAMETER (L=3)
      PARAMETER (M=10)
      PARAMETER (N=15)
      ...
      REAL*4 IN(N,M,L),EXP_X(N,M,L)
      COMPLEX*8 OUT(OUT_N,M,L)
      REAL*4 ERR,SCALE
...
!
      PRINT *, 'Check results'
      CALL CHECK_RESULT_S(IN,EXP_X,N*M*L,ERR)
      PRINT *, 'Accuracy=',ERR
