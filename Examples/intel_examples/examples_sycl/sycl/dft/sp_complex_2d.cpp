...
static int verify_fwd(float* data, int N1, int N2, int H1, int H2) {
...
static int verify_bwd(float* data, int N1, int N2, int H1, int H2) {
    // Note: this simple error bound doesn't take into account error of
    //       input data
    float errthr = 5.0f * logf((float) N2*N1) / logf(2.0f) * FLT_EPSILON;
    std::cout << "\t\tVerify the result, errthr = " << errthr << std::endl;
...
int run_dft_example(cl::sycl::device &dev) {
    ...
    try {
        ...
        float* in = (float*) mkl_calloc(N2*N1*2, sizeof(float), 64);
        init(in, N1, N2, H1, H2);
        cl::sycl::buffer<float, 1> inBuffer(in, cl::sycl::range<1>(N2*N1*2));
        ...
        oneapi::mkl::dft::compute_forward(desc, inBuffer);
        {
          ...
          result = verify_fwd(inAcc.get_pointer(), N1, N2, H1, H2);
        }
        if (result != FAILURE) {
            oneapi::mkl::dft::compute_backward(desc, inBuffer);
            auto inAcc = inBuffer.get_access<cl::sycl::access::mode::read>();
            result = verify_bwd(inAcc.get_pointer(), N1, N2, H1, H2);
        }
        ...
        fwd.wait();
        result = verify_fwd(in_usm, N1, N2, H1, H2);
