...
*       API oneapi::mkl::dft to perform 3-D Double Precision Complex to Complex batched
...
static void init(double *data, int N1, int N2, int N3, int BATCH, int H1, int H2, int H3)
{
            ...
            for (int n2 = 0; n2 < N2; ++n2) {
                for (int n1 = 0; n1 < N1; ++n1) {
                    double phase = TWOPI * (moda(n1, H1, N1) / N1
                                                + moda(n2, H2, N2) / N2
                                                + moda(n3, H3, N3) / N3);
                    int index = k*2*N1*N2*N3 + 2*(n3*S3 + n2*S2 + n1*S1);
                    data[index+0] = cos(phase) / (N3*N2*N1);
                    data[index+1] = sin(phase) / (N3*N2*N1);
                }
            }
        }
    }
...
static int verify_fwd(cl::sycl::buffer<double, 1> dataBuffer, int N1, int N2, int N3, int BATCH, int H1, int H2, int H3) {
    // Note: this simple error bound doesn't take into account error of
    //       input data
    double errthr = 5.0 * log((double) N3*N2*N1) / log(2.0) * DBL_EPSILON;
...
static int verify_bwd(cl::sycl::buffer<double, 1> dataBuffer, int N1, int N2, int N3, int BATCH, int H1, int H2, int H3) {
    // Note: this simple error bound doesn't take into account error of
    //       input data
    double errthr = 5.0 * log((double) N3*N2*N1) / log(2.0) * DBL_EPSILON;
    std::cout << "\t\tVerify the result, errthr = " << errthr << std::endl;
            ...
            for (int n2 = 0; n2 < N2; n2++) {
                for (int n1 = 0; n1 < N1; n1++) {
                    double phase = TWOPI * (moda(n1, H1, N1) / N1
                                                + moda(n2, H2, N2) / N2
                                                + moda(n3, H3, N3) / N3);
                    double re_exp = cos(phase) / (N3*N2*N1);
                    double im_exp = sin(phase) / (N3*N2*N1);
...
int run_dft_example(cl::sycl::device &dev) {
    ...
    double* in = (double*) mkl_calloc(BATCH*N3*N2*N1*2, sizeof(double), 64);
    init(in, N1, N2, N3, BATCH, H1, H2, H3);
    cl::sycl::buffer<double, 1> inBuffer(in, cl::sycl::range<1>(BATCH*N3*N2*N1*2));
    ...
    try {
        ...
        desc.set_value(oneapi::mkl::dft::config_param::FWD_DISTANCE, N1*N2*N3);
        desc.set_value(oneapi::mkl::dft::config_param::BWD_DISTANCE, N1*N2*N3);
        desc.set_value(oneapi::mkl::dft::config_param::BACKWARD_SCALE, (1.0/(N1*N2*N3)));
        desc.commit(queue);

        oneapi::mkl::dft::compute_forward(desc, inBuffer);
        result = verify_fwd(inBuffer, N1, N2, N3, BATCH, H1, H2, H3);

        if (result != FAILURE) {
            oneapi::mkl::dft::compute_backward(desc, inBuffer);
            result = verify_bwd(inBuffer, N1, N2, N3, BATCH, H1, H2, H3);
        }
    }
    catch(cl::sycl::exception const& e) {
...
void print_example_banner() {
    std::cout << "" << std::endl;
    std::cout << "########################################################################" << std::endl;
    std::cout << "# 3D FFT Complex-Complex Double-Precision Batched Example: " << std::endl;
    std::cout << "# " << std::endl;
...
int main() {
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            std::cout << "\tRunning with double precision complex-to-complex batched 3-D FFT:" << std::endl;
