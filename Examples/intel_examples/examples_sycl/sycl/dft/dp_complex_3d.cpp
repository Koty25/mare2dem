...
*       API oneapi::mkl::dft to perform 3-D Double Precision Complex to Complex
...
static void init(double *data, int N1, int N2, int N3, int H1, int H2, int H3)
{
...
static int verify_fwd(double* data, int N1, int N2, int N3, int H1, int H2, int H3) {
...
static int verify_bwd(double* data, int N1, int N2, int N3, int H1, int H2, int H3) {
    // Note: this simple error bound doesn't take into account error of
    //       input data
    double errthr = 5.0 * log((double) N3*N2*N1) / log(2.0) * DBL_EPSILON;
    std::cout << "\t\tVerify the result, errthr = " << errthr << std::endl;
...
int run_dft_example(cl::sycl::device &dev) {
    ...
    try {
        ...
        double* in = (double*) mkl_calloc(N3*N2*N1*2, sizeof(double), 64);
        init(in, N1, N2, N3, H1, H2, H3);
        cl::sycl::buffer<double, 1> inBuffer(in, cl::sycl::range<1>(N3*N2*N1*2));
        inBuffer.set_write_back(false);

        // Setting up USM and initialization
        double *in_usm = (double*) malloc_shared(N3*N2*N1*2*sizeof(double), queue.get_device(), queue.get_context());
...
void print_example_banner() {
    std::cout << "" << std::endl;
    std::cout << "########################################################################" << std::endl;
    std::cout << "# 3D FFT Complex-Complex Double-Precision Example: " << std::endl;
    std::cout << "# " << std::endl;
...
int main() {
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            std::cout << "\tRunning with double precision complex-to-complex 3-D FFT:" << std::endl;
