...
*       API oneapi::mkl::dft to perform 3-D Single Precision Complex to Complex batched
...
static void init(float *data, int N1, int N2, int N3, int BATCH, int H1, int H2, int H3)
{
...
static int verify_fwd(cl::sycl::buffer<float, 1> dataBuffer, int N1, int N2, int N3, int BATCH, int H1, int H2, int H3) {
...
static int verify_bwd(cl::sycl::buffer<float, 1> dataBuffer, int N1, int N2, int N3, int BATCH, int H1, int H2, int H3) {
    // Note: this simple error bound doesn't take into account error of
    //       input data
    float errthr = 5.0f * logf((float) N3*N2*N1) / logf(2.0f) * FLT_EPSILON;
    std::cout << "\t\tVerify the result, errthr = " << errthr << std::endl;
...
int run_dft_example(cl::sycl::device &dev) {
    ...
    float* in = (float*) mkl_calloc(BATCH*N3*N2*N1*2, sizeof(float), 64);
    init(in, N1, N2, N3, BATCH, H1, H2, H3);
    cl::sycl::buffer<float, 1> inBuffer(in, cl::sycl::range<1>(BATCH*N3*N2*N1*2));
...
void print_example_banner() {
    std::cout << "" << std::endl;
    std::cout << "########################################################################" << std::endl;
    std::cout << "# 3D FFT Complex-Complex Single-Precision Batched Example: " << std::endl;
    std::cout << "# " << std::endl;
...
int main() {
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            std::cout << "\tRunning with single precision complex-to-complex batched 3-D FFT:" << std::endl;
