...
*       API oneapi::mkl::dft to perform 2-D Single Precision Real to Complex
...
static void init_r(float *data, int N1, int N2, int H1, int H2)
{
    ...
    int S1 = 1, S2 = (N1/2+1)*2;

    float factor =
        ((2 * (N1 - H1) % N1 == 0) &&
         (2 * (N2 - H2) % N2 == 0)) ? 1.0f : 2.0f;
                    ...
                    int N1, int N2, int H1, int H2)
{
    ...
    //       input data
    float errthr = 2.5f * logf((float) N2*N1) / logf(2.0f) * FLT_EPSILON;
...
static void init_c(float *data, int N1, int N2, int H1, int H2)
{
    // Generalized strides for row-major addressing of data
    int S1 = 1, S2 = N1/2+1;
    ...
    for (int n2 = 0; n2 < N2; n2++) {
        for (int n1 = 0; n1 < N1/2+1; n1++) {
            float phase = moda(n1, H1, N1) / N1 +
                                moda(n2, H2, N2) / N2;
            int index = n2*S2 + n1*S1;
            data[index*2+0] =  cosf(TWOPI * phase) / (N2*N1);
            data[index*2+1] = -sinf(TWOPI * phase) / (N2*N1);
        }
    }
}
...
int run_dft_forward_example(cl::sycl::device &dev) {
    ...
    try {
        ...
        float* x = (float*) mkl_calloc(N1*N2*2 + 2, sizeof(float), 64);
        init_r(x, N1, N2, H1, H2);
        cl::sycl::buffer<float, 1> xBuffer(x, cl::sycl::range<1>(N1*N2*2 + 2));
        xBuffer.set_write_back(false);

        // Setting up USM and initialization
        float *x_usm = (float*) malloc_shared((N1*N2*2 + 2)*sizeof(float), queue.get_device(), queue.get_context());
        ...
        result = verify_c(xAcc.get_pointer(), N1, N2, H1, H2);
        ...
        fwd.wait();
        result = verify_c(x_usm, N1, N2, H1, H2);
...
int run_dft_backward_example(cl::sycl::device &dev) {
    ...
    try {
        ...
        auto xAcc = xBuffer.get_access<cl::sycl::access::mode::read>();
        status = verify_r(xAcc.get_pointer(), N1, N2, H1, H2);
        ...
        bwd.wait();
        status = verify_r(x_usm, N1, N2, H1, H2);
...
//
//  For each device selected and each supported data type, Basic_Sp_R2C_2D_FFTExample
...
int main() {
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            std::cout << "\tRunning with single precision real-to-complex 2-D FFT:" << std::endl;
