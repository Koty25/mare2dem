                    ...
                    int H1, int H2, int H3)
{
    ...
    //       input data
    double errthr = 2.5 * log((double) N3*N2*N1) / log(2.0) * DBL_EPSILON;
        ...
        for (int n2 = 0; n2 < N2; n2++) {
            for (int n1 = 0; n1 < N1/2+1; n1++) {
                double phase = moda(n1, H1, N1) / N1 +
                                     ...
                                     moda(n3, H3, N3) / N3;
                int index = n3*S3 + n2*S2 + n1*S1;
                data[index*2+0] =  cos(TWOPI * phase) / (N3*N2*N1);
                data[index*2+1] = -sin(TWOPI * phase) / (N3*N2*N1);
            }
        }
    }
}
...
int run_dft_forward_example(cl::sycl::device &dev) {
    ...
    try {
        ...
        double* x = (double*) mkl_calloc(N1*N2*N3*2 + 2, sizeof(double), 64);
        init_r(x, N1, N2, N3, H1, H2, H3);
        cl::sycl::buffer<double, 1> xBuffer(x, cl::sycl::range<1>(N1*N2*N3*2 + 2));
        xBuffer.set_write_back(false);

        // Setting up USM and initialization
        double *x_usm = (double*) malloc_shared((N1*N2*N3*2 + 2)*sizeof(double), queue.get_device(), queue.get_context());
...
void print_example_banner() {
    std::cout << "" << std::endl;
    std::cout << "########################################################################" << std::endl;
    std::cout << "# 3D FFT Real-Complex Double-Precision Example: " << std::endl;
    std::cout << "# " << std::endl;
...
//
//  For each device selected and each supported data type, Basic_Dp_R2C_3D_FFTExample
...
int main() {
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            std::cout << "\tRunning with double precision real-to-complex 3-D FFT:" << std::endl;
