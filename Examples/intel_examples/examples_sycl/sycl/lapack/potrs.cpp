...
void run_potrs_example(const cl::sycl::device& device)
{
    ...
    try {
        ...
        using buffer_t = cl::sycl::buffer<data_t, 1>;
        buffer_t A_buffer{A.data(), A_size};
        buffer_t B_buffer{B.data(), B_size};
        buffer_t potrf_scratchpad{potrf_scratchpad_size};
        buffer_t potrs_scratchpad{potrs_scratchpad_size};

        // Perform factorization
        oneapi::mkl::lapack::potrf(queue, uplo, n, A_buffer, lda, potrf_scratchpad, potrf_scratchpad_size);

        // Perform solve operation
        oneapi::mkl::lapack::potrs(queue, uplo, n, nrhs, A_buffer, lda,  B_buffer, ldb, potrs_scratchpad, potrs_scratchpad_size);
