 ...
 *
 *  Content:
 *            oneapi::mkl::vm::ceil example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vCeilAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x40D9B85C) }, { UINT32_C(0x40E00000) } }, //  0: vsCeil ( 6.80375481      ) = ( 7               );
{ { UINT32_C(0xC007309A) }, { UINT32_C(0xC0000000) } }, //  1: vsCeil ( -2.1123414      ) = ( -2              );
{ { UINT32_C(0x40B52EFA) }, { UINT32_C(0x40C00000) } }, //  2: vsCeil ( 5.66198444      ) = ( 6               );
{ { UINT32_C(0x40BF006A) }, { UINT32_C(0x40C00000) } }, //  3: vsCeil ( 5.96880054      ) = ( 6               );
 }
...
{ { UINT64_C(0x401B370B60E66E18) }, { UINT64_C(0x401C000000000000) } }, //  0: vdCeil ( 6.80375434309419092       ) = ( 7                         );
{ { UINT64_C(0xC000E6134801CC26) }, { UINT64_C(0xC000000000000000) } }, //  1: vdCeil ( -2.11234146361813924      ) = ( -2                        );
{ { UINT64_C(0x4016A5DF421D4BBE) }, { UINT64_C(0x4018000000000000) } }, //  2: vdCeil ( 5.66198447517211711       ) = ( 6                         );
{ { UINT64_C(0x4017E00D485FC01A) }, { UINT64_C(0x4018000000000000) } }, //  3: vdCeil ( 5.96880066952146571       ) = ( 6                         );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::ceil(main_queue, VLEN, varg1, vres1, no_deps, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vCeilAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vCeilAccuracyLiteTest<double, double>(my_dev);
    } else {
