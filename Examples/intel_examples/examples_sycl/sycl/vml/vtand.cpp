 ...
 *
 *  Content:
 *            oneapi::mkl::vm::tand example program text (SYCL interface)
 *
 *******************************************************************************/
...
static ulp_table_type ulp_table = {
    {MAX_HA_ULP_S, 512.0},   {MAX_LA_ULP_S, 512.0},   {MAX_EP_ULP_S, 5000.0},
...
bool vTandAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x442A1808) }, { UINT32_C(0xBF53F756) } }, //  0: vsTand ( 680.375488      ) = ( -0.827992797    );
{ { UINT32_C(0xC3533BF0) }, { UINT32_C(0xBF1B3F64) } }, //  1: vsTand ( -211.234131     ) = ( -0.606436014    );
{ { UINT32_C(0x440D8CB4) }, { UINT32_C(0x3EFBEB1E) } }, //  2: vsTand ( 566.198486      ) = ( 0.492028177     );
{ { UINT32_C(0x44153854) }, { UINT32_C(0x3FC433EC) } }, //  3: vsTand ( 596.880127      ) = ( 1.53283453      );
 }
...
{ { UINT64_C(0x40854300E3B40602) }, { UINT64_C(0xBFEA7EEE13BC9A32) } }, //  0: vdTand ( 680.375434309419006       ) = ( -0.827994383381172705     );
{ { UINT64_C(0xC06A677E2082CEFC) }, { UINT64_C(0xBFE367ED418AB69C) } }, //  1: vdTand ( -211.234146361813941      ) = ( -0.606436374675201062     );
{ { UINT64_C(0x4081B1966BA6E32C) }, { UINT64_C(0x3FDF7D60444F3889) } }, //  2: vdTand ( 566.198447517211662       ) = ( 0.492027346332982563      );
{ { UINT64_C(0x4082A70A608ACE14) }, { UINT64_C(0x3FF88679C577E974) } }, //  3: vdTand ( 596.880066952146535       ) = ( 1.53283097396942214       );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::tand(main_queue, varg1.size(), in1, out1, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vTandAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vTandAccuracyLiteTest<double, double>(my_dev);
    } else {
