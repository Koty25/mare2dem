 ...
 *
 *  Content:
 *            oneapi::mkl::vm::nextafter example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vNextAfterAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x40D9B85C) }, { UINT32_C(0xC007309A) }, { UINT32_C(0x40D9B85B) } }, //  0: vsNextAfter ( 6.80375481     , -2.1123414      ) = ( 6.80375433      );
{ { UINT32_C(0x40B52EFA) }, { UINT32_C(0x40BF006A) }, { UINT32_C(0x40B52EFB) } }, //  1: vsNextAfter ( 5.66198444     , 5.96880054      ) = ( 5.66198492      );
{ { UINT32_C(0x4103BA28) }, { UINT32_C(0xC0C1912F) }, { UINT32_C(0x4103BA27) } }, //  2: vsNextAfter ( 8.2329483      , -6.04897261     ) = ( 8.23294735      );
{ { UINT32_C(0xC052EA36) }, { UINT32_C(0x40ABAABC) }, { UINT32_C(0xC052EA35) } }, //  3: vsNextAfter ( -3.2955451     , 5.3645916       ) = ( -3.29554486     );
 }
...
{ { UINT64_C(0x401B370B60E66E18) }, { UINT64_C(0xC000E6134801CC26) }, { UINT64_C(0x401B370B60E66E17) } }, //  0: vdNextAfter ( 6.80375434309419092      , -2.11234146361813924      ) = ( 6.80375434309419003       );
{ { UINT64_C(0x4016A5DF421D4BBE) }, { UINT64_C(0x4017E00D485FC01A) }, { UINT64_C(0x4016A5DF421D4BBF) } }, //  1: vdNextAfter ( 5.66198447517211711      , 5.96880066952146571       ) = ( 5.661984475172118         );
{ { UINT64_C(0x40207744D998EE8A) }, { UINT64_C(0xC0183225E080644C) }, { UINT64_C(0x40207744D998EE89) } }, //  2: vdNextAfter ( 8.23294715873568705      , -6.04897261413232101      ) = ( 8.23294715873568528       );
{ { UINT64_C(0xC00A5D46A314BA8E) }, { UINT64_C(0x4015755793FAEAB0) }, { UINT64_C(0xC00A5D46A314BA8D) } }, //  3: vdNextAfter ( -3.2955448857022196      , 5.36459189623808186       ) = ( -3.29554488570221915      );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    oneapi::mkl::vm::nextafter(main_queue, VLEN, varg1, varg2, vres1, no_deps,
                       accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vNextAfterAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vNextAfterAccuracyLiteTest<double, double>(my_dev);
    } else {
