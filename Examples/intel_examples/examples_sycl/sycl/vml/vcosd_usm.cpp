 ...
 *
 *  Content:
 *            oneapi::mkl::vm::cosd example program text (SYCL interface)
 *
 *******************************************************************************/
...
static ulp_table_type ulp_table = {
    {MAX_HA_ULP_S, 5000.0},  {MAX_LA_ULP_S, 5000.0},  {MAX_EP_ULP_S, 5000.0},
...
bool vCosdAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x442A1808) }, { UINT32_C(0x3F452E7B) } }, //  0: vsCosd ( 680.375488      ) = ( 0.770240486     );
{ { UINT32_C(0xC3533BF0) }, { UINT32_C(0xBF5AE4EB) } }, //  1: vsCosd ( -211.234131     ) = ( -0.855055511    );
{ { UINT32_C(0x440D8CB4) }, { UINT32_C(0xBF65B37D) } }, //  2: vsCosd ( 566.198486      ) = ( -0.897270024    );
{ { UINT32_C(0x44153854) }, { UINT32_C(0xBF0BE061) } }, //  3: vsCosd ( 596.880127      ) = ( -0.5463925      );
 }
...
{ { UINT64_C(0x40854300E3B40602) }, { UINT64_C(0x3FE8A5CE182B4917) } }, //  0: vdCosd ( 680.375434309419006       ) = ( 0.770239875035516941      );
{ { UINT64_C(0xC06A677E2082CEFC) }, { UINT64_C(0xBFEB5C9D1A82A314) } }, //  1: vdCosd ( -211.234146361813941      ) = ( -0.85505538156312122      );
{ { UINT64_C(0x4081B1966BA6E32C) }, { UINT64_C(0xBFECB67045A4F282) } }, //  2: vdCosd ( 566.198447517211662       ) = ( -0.897270332359383582     );
{ { UINT64_C(0x4082A70A608ACE14) }, { UINT64_C(0xBFE17C0DF1D13588) } }, //  3: vdCosd ( 596.880066952146535       ) = ( -0.546393368052734196     );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::cosd(main_queue, VLEN, varg1, vres1, no_deps, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vCosdAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vCosdAccuracyLiteTest<double, double>(my_dev);
    } else {
