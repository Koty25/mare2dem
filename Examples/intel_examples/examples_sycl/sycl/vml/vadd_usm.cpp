...
bool vAddAccuracyLiteTest(const cl::sycl::device &dev) {
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision complex data type:"
                << std::endl;
      errs += vAddAccuracyLiteTest<std::complex<float>, std::complex<float>>(
          my_dev);

      std::cout << "\tRunning with double precision complex data type:"
                << std::endl;
      errs += vAddAccuracyLiteTest<std::complex<double>, std::complex<double>>(
          my_dev);
