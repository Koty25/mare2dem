 ...
 *
 *  Content:
 *            oneapi::mkl::vm::invcbrt example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vInvCbrtAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x40D9B85C) }, { UINT32_C(0x3F0719BD) } }, //  0: vsInvCbrt ( 6.80375481      ) = ( 0.527736485     );
{ { UINT32_C(0xC007309A) }, { UINT32_C(0xBF4784FB) } }, //  1: vsInvCbrt ( -2.1123414      ) = ( -0.779372871    );
{ { UINT32_C(0x40B52EFA) }, { UINT32_C(0x3F0FA1B9) } }, //  2: vsInvCbrt ( 5.66198444      ) = ( 0.561061442     );
{ { UINT32_C(0x40BF006A) }, { UINT32_C(0x3F0D2095) } }, //  3: vsInvCbrt ( 5.96880054      ) = ( 0.551278412     );
 }
...
{ { UINT64_C(0x401B370B60E66E18) }, { UINT64_C(0x3FE0E337A3487F51) } }, //  0: vdInvCbrt ( 6.80375434309419092       ) = ( 0.527736491119847995      );
{ { UINT64_C(0xC000E6134801CC26) }, { UINT64_C(0xBFE8F09F674D7307) } }, //  1: vdInvCbrt ( -2.11234146361813924      ) = ( -0.779372884524122678     );
{ { UINT64_C(0x4016A5DF421D4BBE) }, { UINT64_C(0x3FE1F4372CC88437) } }, //  2: vdInvCbrt ( 5.66198447517211711       ) = ( 0.561061465709037033      );
{ { UINT64_C(0x4017E00D485FC01A) }, { UINT64_C(0x3FE1A412983CABD1) } }, //  3: vdInvCbrt ( 5.96880066952146571       ) = ( 0.551278397882351112      );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::invcbrt(main_queue, varg1.size(), in1, out1, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vInvCbrtAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vInvCbrtAccuracyLiteTest<double, double>(my_dev);
    } else {
