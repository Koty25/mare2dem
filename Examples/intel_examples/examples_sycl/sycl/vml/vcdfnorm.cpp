 ...
 *
 *  Content:
 *            oneapi::mkl::vm::cdfnorm example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vCdfNormAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x406150E8) }, { UINT32_C(0x3F7FF1E4) } }, //  0: vsCdfNorm ( 3.52056313      ) = ( 0.999784708     );
{ { UINT32_C(0x400BB8B6) }, { UINT32_C(0x3F7C48EA) } }, //  1: vsCdfNorm ( 2.18314886      ) = ( 0.98548758      );
{ { UINT32_C(0x40565AE5) }, { UINT32_C(0x3F7FE574) } }, //  2: vsCdfNorm ( 3.34929776      ) = ( 0.999594927     );
{ { UINT32_C(0x40594CED) }, { UINT32_C(0x3F7FE98A) } }, //  3: vsCdfNorm ( 3.39532018      ) = ( 0.999657273     );
 }
...
{ { UINT64_C(0x400C2A1D0378543A) }, { UINT64_C(0x3FEFFE3C73412180) } }, //  0: vdCdfNorm ( 3.52056315146412846       ) = ( 0.999784684282573721      );
{ { UINT64_C(0x40017716B532EE2E) }, { UINT64_C(0x3FEF891D3BBE3379) } }, //  1: vdCdfNorm ( 2.18314878045727934       ) = ( 0.985487572370046583      );
{ { UINT64_C(0x400ACB5C93D596B9) }, { UINT64_C(0x3FEFFCAE7A716C4F) } }, //  2: vdCdfNorm ( 3.34929767127581757       ) = ( 0.999594916483497076      );
{ { UINT64_C(0x400B299D95B6533B) }, { UINT64_C(0x3FEFFD3137C6318B) } }, //  3: vdCdfNorm ( 3.39532010042821986       ) = ( 0.999657257970782864      );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::cdfnorm(main_queue, varg1.size(), in1, out1, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vCdfNormAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vCdfNormAccuracyLiteTest<double, double>(my_dev);
    } else {
