 ...
 *
 *  Content:
 *            oneapi::mkl::vm::logb example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vLogbAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x41093E24) }, { UINT32_C(0x40400000) } }, //  0: vsLogb ( 8.57767105      ) = ( 3               );
{ { UINT32_C(0x4093852F) }, { UINT32_C(0x40000000) } }, //  1: vsLogb ( 4.61000776      ) = ( 2               );
{ { UINT32_C(0x41011D03) }, { UINT32_C(0x40400000) } }, //  2: vsLogb ( 8.06958294      ) = ( 3               );
{ { UINT32_C(0x41034C40) }, { UINT32_C(0x40400000) } }, //  3: vsLogb ( 8.20611572      ) = ( 3               );
 }
...
{ { UINT64_C(0x402127C473A3E923) }, { UINT64_C(0x4008000000000000) } }, //  0: vdLogb ( 8.57767068267691535       ) = ( 3                         );
{ { UINT64_C(0x401270A5F32DAE19) }, { UINT64_C(0x4000000000000000) } }, //  1: vdLogb ( 4.6100080486899282        ) = ( 2                         );
{ { UINT64_C(0x402023A0651C4741) }, { UINT64_C(0x4008000000000000) } }, //  2: vdLogb ( 8.06958309145159269       ) = ( 3                         );
{ { UINT64_C(0x40206988134D9FDD) }, { UINT64_C(0x4008000000000000) } }, //  3: vdLogb ( 8.20611629793705255       ) = ( 3                         );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::logb(main_queue, VLEN, varg1, vres1, no_deps, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vLogbAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vLogbAccuracyLiteTest<double, double>(my_dev);
    } else {
