 ...
 *
 *  Content:
 *            oneapi::mkl::vm::atanpi example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vAtanpiAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x40D9B85C) }, { UINT32_C(0x3EE83776) } }, //  0: vsAtanpi ( 6.80375481      ) = ( 0.453548133     );
{ { UINT32_C(0xC007309A) }, { UINT32_C(0xBEB7F0E1) } }, //  1: vsAtanpi ( -2.1123414      ) = ( -0.359259635    );
{ { UINT32_C(0x40B52EFA) }, { UINT32_C(0x3EE38282) } }, //  2: vsAtanpi ( 5.66198444      ) = ( 0.444355071     );
{ { UINT32_C(0x40BF006A) }, { UINT32_C(0x3EE4F263) } }, //  3: vsAtanpi ( 5.96880054      ) = ( 0.447161764     );
 }
...
{ { UINT64_C(0x401B370B60E66E18) }, { UINT64_C(0x3FDD06EEC5825AEC) } }, //  0: vdAtanpi ( 6.80375434309419092       ) = ( 0.453548138504102694      );
{ { UINT64_C(0xC000E6134801CC26) }, { UINT64_C(0xBFD6FE1C2CE0567B) } }, //  1: vdAtanpi ( -2.11234146361813924      ) = ( -0.359259647202044341     );
{ { UINT64_C(0x4016A5DF421D4BBE) }, { UINT64_C(0x3FDC7050395B41AB) } }, //  2: vdAtanpi ( 5.66198447517211711       ) = ( 0.444355064403704147      );
{ { UINT64_C(0x4017E00D485FC01A) }, { UINT64_C(0x3FDC9E4C683DEF72) } }, //  3: vdAtanpi ( 5.96880066952146571       ) = ( 0.447161771582378909      );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::atanpi(main_queue, varg1.size(), in1, out1, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vAtanpiAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vAtanpiAccuracyLiteTest<double, double>(my_dev);
    } else {
