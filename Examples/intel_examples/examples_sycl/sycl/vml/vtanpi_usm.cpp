 ...
 *
 *  Content:
 *            oneapi::mkl::vm::tanpi example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vTanpiAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x40D9B85C) }, { UINT32_C(0xBF356B88) } }, //  0: vsTanpi ( 6.80375481      ) = ( -0.708672047    );
{ { UINT32_C(0xC007309A) }, { UINT32_C(0xBEBC98D5) } }, //  1: vsTanpi ( -2.1123414      ) = ( -0.368353516    );
{ { UINT32_C(0x40B52EFA) }, { UINT32_C(0xBFE56E6A) } }, //  2: vsTanpi ( 5.66198444      ) = ( -1.79243207     );
{ { UINT32_C(0x40BF006A) }, { UINT32_C(0xBDC961CD) } }, //  3: vsTanpi ( 5.96880054      ) = ( -0.0983310714   );
 }
...
{ { UINT64_C(0x401B370B60E66E18) }, { UINT64_C(0xBFE6AD759D62B54F) } }, //  0: vdTanpi ( 6.80375434309419092       ) = ( -0.708674247163495141     );
{ { UINT64_C(0xC000E6134801CC26) }, { UINT64_C(0xBFD7931B8E7CDD8B) } }, //  1: vdTanpi ( -2.11234146361813924      ) = ( -0.368353737972449025     );
{ { UINT64_C(0x4016A5DF421D4BBE) }, { UINT64_C(0xBFFCADCCD759EC08) } }, //  2: vdTanpi ( 5.66198447517211711       ) = ( -1.79243167993149832      );
{ { UINT64_C(0x4017E00D485FC01A) }, { UINT64_C(0xBFB92C330946EDEC) } }, //  3: vdTanpi ( 5.96880066952146571       ) = ( -0.0983306787156192352    );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::tanpi(main_queue, VLEN, varg1, vres1, no_deps, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vTanpiAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vTanpiAccuracyLiteTest<double, double>(my_dev);
    } else {
