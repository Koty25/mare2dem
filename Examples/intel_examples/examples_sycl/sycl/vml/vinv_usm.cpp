 ...
 *
 *  Content:
 *            oneapi::mkl::vm::inv example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vInvAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x40D9B85C) }, { UINT32_C(0x3E168150) } }, //  0: vsInv ( 6.80375481      ) = ( 0.146977663     );
{ { UINT32_C(0xC007309A) }, { UINT32_C(0xBEF26293) } }, //  1: vsInv ( -2.1123414      ) = ( -0.473408312    );
{ { UINT32_C(0x40B52EFA) }, { UINT32_C(0x3E34DAF6) } }, //  2: vsInv ( 5.66198444      ) = ( 0.17661652      );
{ { UINT32_C(0x40BF006A) }, { UINT32_C(0x3E2B8F0B) } }, //  3: vsInv ( 5.96880054      ) = ( 0.167537853     );
 }
...
{ { UINT64_C(0x401B370B60E66E18) }, { UINT64_C(0x3FC2D02A1B61C64B) } }, //  0: vdInv ( 6.80375434309419092       ) = ( 0.146977675790866819      );
{ { UINT64_C(0xC000E6134801CC26) }, { UINT64_C(0xBFDE4C525D09E911) } }, //  1: vdInv ( -2.11234146361813924      ) = ( -0.473408308847539649     );
{ { UINT64_C(0x4016A5DF421D4BBE) }, { UINT64_C(0x3FC69B5EC2D437AE) } }, //  2: vdInv ( 5.66198447517211711       ) = ( 0.176616521006903893      );
{ { UINT64_C(0x4017E00D485FC01A) }, { UINT64_C(0x3FC571E14E0D800C) } }, //  3: vdInv ( 5.96880066952146571       ) = ( 0.167537844764411026      );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::inv(main_queue, VLEN, varg1, vres1, no_deps, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vInvAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vInvAccuracyLiteTest<double, double>(my_dev);
    } else {
