 ...
 *
 *  Content:
 *            oneapi::mkl::vm::tgamma example program text (SYCL interface)
 *
 *******************************************************************************/
...
bool vTGammaAccuracyLiteTest(const cl::sycl::device &dev) {
...
{ { UINT32_C(0x4083DF80) }, { UINT32_C(0x40DFFC58) } }, //  0: vsTGamma ( 4.12103271      ) = ( 6.99955368      );
{ { UINT32_C(0x3FD5A544) }, { UINT32_C(0x3F6734B2) } }, //  1: vsTGamma ( 1.66910601      ) = ( 0.903147817     );
{ { UINT32_C(0x4073A6A4) }, { UINT32_C(0x40977CDA) } }, //  2: vsTGamma ( 3.80704594      ) = ( 4.73399067      );
{ { UINT32_C(0x40790D08) }, { UINT32_C(0x40A7CDD7) } }, //  3: vsTGamma ( 3.89142036      ) = ( 5.24387693      );
 }
...
{ { UINT64_C(0x40107BEFEDD8F7E0) }, { UINT64_C(0x401BFF8A5E5FB880) } }, //  0: vdTGamma ( 4.12103244435090232       ) = ( 6.9995512720034867        );
{ { UINT64_C(0x3FFAB4A898656952) }, { UINT64_C(0x3FECE696513F6393) } }, //  1: vdTGamma ( 1.66910609750501182       ) = ( 0.903147848784202956      );
{ { UINT64_C(0x400E74D4645CE9A8) }, { UINT64_C(0x4012EF9AEA1EC930) } }, //  2: vdTGamma ( 3.80704573067233198       ) = ( 4.73398938954260018       );
{ { UINT64_C(0x400F21A0E7CE4342) }, { UINT64_C(0x4014F9BA8CE68C6E) } }, //  3: vdTGamma ( 3.89142018411840329       ) = ( 5.24387569577366541       );
 }
...
,
  ...
  for (int acc = 0; acc < ACCURACY_NUM; ++acc) {
    ...
    // Run VM function
    oneapi::mkl::vm::tgamma(main_queue, varg1.size(), in1, out1, accuracy_mode[acc]);
  ...
  for (auto dev_type : list_of_devices) {
    ...
    if (my_dev_is_found) {
      ...
      std::cout << "\tRunning with single precision real data type:"
                << std::endl;
      errs += vTGammaAccuracyLiteTest<float, float>(my_dev);

      std::cout << "\tRunning with double precision real data type:"
                << std::endl;
      errs += vTGammaAccuracyLiteTest<double, double>(my_dev);
    } else {
