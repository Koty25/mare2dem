...
*       DPCPP API oneapi::mkl::sparse::gemvdot to perform
*general
...
*
*
*       The supported floating point data types for gemvdot matrix data are:
*           float
*           double
*
*
...
int run_sparse_matrix_vector_multiply_dot_product_example(const cl::sycl::device &dev)
{
        ...
        for (std::exception_ptr const &e : exceptions) {
            try {
                ...
                std::cout << "Caught asynchronous SYCL "
                             "exception during sparse::gemvdot:\n"
                          << e.what() << std::endl;
            }
        }
    };
    ...
    cl::sycl::buffer<fp, 1> d_buffer(d.data(), d.size());
    ...
    try {
        ...
        oneapi::mkl::sparse::gemvdot(main_queue, oneapi::mkl::transpose::nontrans, alpha, handle,
    ...
    auto res = d_buffer.template get_access<cl::sycl::access::mode::read>();
    ...
    fp d_cal = set_fp_value(fp(0.0), fp(0.0));
...
int main(int argc, char **argv)
{
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            status = run_sparse_matrix_vector_multiply_dot_product_example<float, std::int32_t>(my_dev);
            if(status != 0) return status;

            if (my_dev.get_info<cl::sycl::info::device::double_fp_config>().size() != 0) {
                std::cout << "\tRunning with double precision real data type:" << std::endl;
                status = run_sparse_matrix_vector_multiply_dot_product_example<double, std::int32_t>(my_dev);
                if(status != 0) return status;
            }
        }
        else {
