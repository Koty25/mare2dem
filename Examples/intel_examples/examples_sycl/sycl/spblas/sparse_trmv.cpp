...
*       DPCPP API oneapi::mkl::sparse::trmv to perform
*triangular
...
int run_sparse_triangular_matrix_vector_multiply_example(const cl::sycl::device &dev)
{
    ...
    try {
        ...
        // trmv optimize
        oneapi::mkl::sparse::optimize_trmv(main_queue, oneapi::mkl::uplo::upper,
                                           oneapi::mkl::transpose::nontrans, diag_val, handle);

        // add oneapi::mkl::sparse::trmv to execution queue
        oneapi::mkl::sparse::trmv(main_queue, oneapi::mkl::uplo::upper,
                                  oneapi::mkl::transpose::nontrans, diag_val, alpha, handle,
    ...
    std::cout << "\t\t\ttranspose_val = "
              ...
              << std::endl;
    std::cout << "\t\t\tdiag = " << (diag_val == oneapi::mkl::diag::unit ? "unit" : "nonunit")
              << std::endl;
    ...
    for (intType row = 0; row < nrows; row++) {
        ...
        else if (diag_val == oneapi::mkl::diag::unit) {
            for (intType i = ia[row]; i < ia[row + 1]; i++) {
                if (ja[i] > row)
                    tmp += a[i] * x[ja[i]];
            }
...
void print_example_banner()
{
    ...
    std::cout << "###############################################################"
                 "#########"
              << std::endl;
    std::cout << "# Sparse Triangular Matrix-Vector Multiply Example: " << std::endl;
    std::cout << "# " << std::endl;
...
int main(int argc, char **argv)
{
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        if (my_dev_is_found) {
            ...
            status = run_sparse_triangular_matrix_vector_multiply_example<float, std::int32_t>(my_dev);
            if(status != 0) return status;

            if (my_dev.get_info<cl::sycl::info::device::double_fp_config>().size() != 0) {
                std::cout << "\tRunning with double precision real data type:" << std::endl;
                status = run_sparse_triangular_matrix_vector_multiply_example<double, std::int32_t>(my_dev);
                if(status != 0) return status;
            }
        }
        else {
