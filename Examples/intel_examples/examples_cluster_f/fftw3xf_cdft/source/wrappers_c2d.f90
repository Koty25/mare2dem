...
program dp_c2c_2d
  ...
  integer(C_INTPTR_T), parameter :: N2 = 518
  ...
  if (0 == irank) then
      ...
      print '(" FFT data size: "I0" x "I0)', N1, N2
  ...
  ! allocating array
  local_size = fftw_mpi_local_size_2d(N1, N2, comm, local_n1, local_1_start)
  x_ptr = fftw_alloc_complex(local_size)
  call c_f_pointer(x_ptr, x, [N2, local_n1])

  print '(" FFT local data size on rank "I2": "I3" x "I3)', irank, local_n1, N2

  plan_fwd = fftw_mpi_plan_dft_2d(N1, N2, x, x, comm, &
      FFTW_FORWARD, FFTW_ESTIMATE)
  plan_bwd = fftw_mpi_plan_dft_2d(N1, N2, x, x, comm, &
      FFTW_BACKWARD, FFTW_ESTIMATE)

  ! Note reversed indexing: x(j,i) <- foo(i,j)
  forall (i=1:local_n1, j=1:N2)
      x(j, i) = foo(local_1_start + i, j, N1, N2)
  end forall
...
contains
  ...
  pure complex(C_DOUBLE_COMPLEX) function foo(i, j, N1, N2)
      integer(C_INTPTR_T), intent(in) :: i, j, N1, N2

      foo = CMPLX( sin(1.1d0 * i / N1), cos(1.2d0 * j / N2) )
  end function foo

end program dp_c2c_2d
