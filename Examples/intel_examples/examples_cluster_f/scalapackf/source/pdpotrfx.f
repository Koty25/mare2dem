...
!
!      Performance and validation of PDPOTRF function
!
!*******************************************************************************
!
!===============================================================================
...
!===============================================================================
!
! Example performs Cholesky factorization of a general matrix with validation and
...
! INDXG2P
!
! In this example the matrix generated by PDMATGEN is factorized by PDPOTRF.
! For performance measurement only PDPOTRF execution time is taken into
! account.
!
!*******************************************************************************

      PROGRAM PDPOTRF_DRIVER
*     ==== Parameters ==================================================
...
*     ==== External subroutines ========================================
     ...
     $         PDPOTRF, DESCINIT, IGEBS2D, IGEBR2D, SGEBS2D, SGEBR2D, 
     $         PDLASET, PDLACPY, PDSYRK, CALC_AVERAGE_TIME
*
*     ==== External functions ==========================================
      DOUBLE PRECISION   DSECND, PDLANSY, PDLAMCH
      INTEGER            NUMROC, INDXG2P, ICEIL, ILCM
      EXTERNAL           DSECND, PDLANSY, PDLAMCH, NUMROC, INDXG2P,
     $                   ICEIL, ILCM
*
*     ==== Intrinsic functions =========================================
      INTRINSIC          MAX, DBLE
...
*     Write table header to output
      IF( IAM.EQ.0 ) THEN
         WRITE( NOUT, FMT = 9999 ) 'PDPOTRF performance results'
         WRITE( NOUT, FMT = 9996 )
...
*                 Factor matrix A
                  TIME = DSECND()
                  CALL PDPOTRF( UPLO, N, A, 1, 1, DESCA, INFO )
                  CALL BLACS_BARRIER( ICTXT, 'All' )
                  TIME = DSECND() - TIME
*
                  TIME_ITER( LNCH ) = TIME
...
*                    Copy UPLO part of factorized matrix into a zero matrix
                     CALL PDLASET( 'All', N, N, ZERO, ZERO, A0, 1, 1,
     $                             DESCA0 )
                     CALL PDLACPY( UPLO, N, N, A, 1, 1, DESCA, A0, 1, 1,
     $                             DESCA0 )
*
*                    Regenerate matrix A
                     CALL PDMATGEN( ICTXT, 'Symm', 'Diag', DESCA( M_ ),
...
*                    Calculate norm of the matrix A ( 1 or infinity)
                     ALLOCATE( WORK( LWORK ) )      
                     ANORM = PDLANSY( 'I', UPLO, N, A, 1, 1, DESCA,
     $                                WORK )
*
