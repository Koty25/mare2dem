...
!
!      Performance and validation of PCGEQRF function
!
!*******************************************************************************
!
!===============================================================================
...
! INDXG2P
!
! In this example the matrix generated by PCMATGEN is factorized by PCGEQRF.
! For performance measurement only PCGEQRF execution time is taken into
! account.
!
!*******************************************************************************

      PROGRAM PCGEQRF_DRIVER
*     ==== Parameters ==================================================
...
*     ==== External subroutines ========================================
     ...
     $         PCGEQRF, DESCINIT, IGEBS2D, IGEBR2D, SGEBS2D, SGEBR2D, 
     $         PCLASET, PCLACPY, PCGEMM, PCUNGQR, CALC_AVERAGE_TIME
*
*     ==== External functions ==========================================
...
*     Write table header to output
      IF( IAM.EQ.0 ) THEN
         WRITE( NOUT, FMT = 9999 ) 'PCGEQRF performance results'
         WRITE( NOUT, FMT = 9996 )
...
*                 Ask necessary size of working array for factorization
                  CALL PCGEQRF( M, N, A, 1, 1, DESCA, TAU, ASK_LWORK,-1,
     $                          INFO )
                  LWORK = INT(ASK_LWORK(1))
                  ALLOCATE( WORK( LWORK ) )
*                 Factor matrix A
                  TIME = DSECND()
                  CALL PCGEQRF( M, N, A, 1, 1, DESCA, TAU, WORK,
     $                          LWORK, INFO )
                  CALL BLACS_BARRIER( ICTXT, 'All' )
                  TIME = DSECND() - TIME
                  DEALLOCATE( WORK )
...
*                    Copy R and Q into different matrices
                     CALL PCLASET( 'L', DESCR( M_ ), DESCR( N_ ), ZERO,
     $                             ZERO, R, 1, 1, DESCR )
...
*                    Generate matrix Q from reflections                    
                     CALL PCUNGQR( M, MIN( M, N ), MIN( M, N ), Q, 1, 1,
