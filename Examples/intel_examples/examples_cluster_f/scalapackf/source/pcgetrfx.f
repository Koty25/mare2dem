...
!
!      Performance and validation of PCGETRF function
!
!*******************************************************************************
!
!===============================================================================
...
! INDXG2P
!
! In this example the matrix generated by PCMATGEN is factorized by PCGETRF.
! For performance measurement only PCGETRF execution time is taken into
! account.
!
!*******************************************************************************

      PROGRAM PCGETRF_DRIVER
*     ==== Parameters ==================================================
...
*     ==== External subroutines ========================================
     ...
     $         PCGETRF, DESCINIT, IGEBS2D, IGEBR2D, SGEBS2D, SGEBR2D, 
     $         PCLASET, PCLACPY, PCGEMM, PCLAPIV, DESCSET,
     $         CALC_AVERAGE_TIME
*
*     ==== External functions ==========================================
      ...
      EXTERNAL           DSECND, PCLANGE, PSLAMCH, NUMROC, INDXG2P
*
*     ==== Intrinsic functions =========================================
...
*     Write table header to output
      IF( IAM.EQ.0 ) THEN
         WRITE( NOUT, FMT = 9999 ) 'PCGETRF performance results'
         WRITE( NOUT, FMT = 9996 )
...
*                 Factor matrix A
                  TIME = DSECND()
                  CALL PCGETRF( M, N, A, 1, 1, DESCA, IPIV, INFO )
                  CALL BLACS_BARRIER( ICTXT, 'All' )
                  TIME = DSECND() - TIME
*
                  TIME_ITER( LNCH ) = TIME
...
*                    Copy U and L into different matrices
                     CALL PCLASET( 'L', DESCU( M_ ), DESCU( N_ ), ZERO,
     $                             ZERO, U, 1, 1, DESCU )
                     CALL PCLACPY( 'U', DESCA( M_ ), DESCA( N_ ), A, 1,
     $                             1, DESCA, U, 1, 1, DESCU )
                     CALL PCLACPY( 'L', DESCA( M_ ), DESCA( N_ ), A, 1,
     $                             1, DESCA, L, 1, 1, DESCL )
                     CALL PCLASET( 'U', DESCL( M_ ), DESCL( N_ ), ZERO,
     $                             ONE,  L, 1, 1, DESCL )
*
*                    Regenerate matrix A
                     CALL PCMATGEN( ICTXT, 'No', 'No', DESCA( M_ ),
...
*                    Calculate norm of the matrix A ( 1 or infinity)
                     ALLOCATE( WORK( LWORK ) )
                     ANORM = PCLANGE( 'I', M, N, A, 1, 1, DESCA, WORK )
*
*                    Compute P*L
     ...
     $                             RSRC_ ), NPROW ) + DESCA( MB_ ) )
                     CALL PCLAPIV( 'B', 'R', 'C', MIN( M, N ), N, L, 1,
     $                             1, DESCL, IPIV, 1, 1, DESCIP, IPWORK)
*
*                    Compute matrix P*L*U-A -> A
                     CALL PCGEMM( 'N', 'N', M, N, MIN( M, N ), ONE,
...
*                    Calculate norm of the matrix P*L*U-A ( 1 or infinity)
                     DIFNORM = PCLANGE( 'I', M, N, A, 1, 1, DESCA, WORK)
                     DEALLOCATE( WORK )
*
