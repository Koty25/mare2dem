...
*
                  IF( IOFFR .GT. IOFFC ) THEN
                     DO 20 J = 1, MB
                        ...
                        A(IK,JK) = DCMPLX( ONE - TWO*PDRAND(IDUMM),
     $                                     ONE - TWO*PDRAND(IDUMM) )
                        IK = IK + 1
   20                CONTINUE
*
                  ELSE IF( IOFFC .EQ. IOFFR ) THEN
                     ...
                     IF( IK .GT. IRNUM ) GO TO 60
                     DO 30 J = 1, I-1
                        A(IK,JK) = DCMPLX( PDRAND(IDUMM), PDRAND(IDUMM))
   30                CONTINUE
                     IF( SYMM ) THEN
                        A(IK,JK) = DCMPLX( ONE - TWO*PDRAND(IDUMM),
     $                                     ONE - TWO*PDRAND(IDUMM) )
                     ELSE
                        A(IK,JK) = DCMPLX( ONE -TWO*PDRAND(IDUMM), ZERO)
                        DUMMY = PDRAND(IDUMM)
                     END IF
                     DO 40 J = 1, MB-I
                        IF( IK+J .GT. IRNUM ) GO TO 60
                        A(IK+J,JK) = DCMPLX( ONE - TWO*PDRAND(IDUMM),
     $                                       ONE - TWO*PDRAND(IDUMM) )
                        IF( HERM ) THEN
