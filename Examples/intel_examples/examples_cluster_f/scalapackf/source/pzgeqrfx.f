...
!
!      Performance and validation of PZGEQRF function
!
!*******************************************************************************
!
!===============================================================================
...
! INDXG2P
!
! In this example the matrix generated by PZMATGEN is factorized by PZGEQRF.
! For performance measurement only PZGEQRF execution time is taken into
! account.
!
!*******************************************************************************

      PROGRAM PZGEQRF_DRIVER
*     ==== Parameters ==================================================
...
*     ==== External subroutines ========================================
     ...
     $         PZGEQRF, DESCINIT, IGEBS2D, IGEBR2D, SGEBS2D, SGEBR2D, 
     $         PZLASET, PZLACPY, PZGEMM, PZUNGQR, CALC_AVERAGE_TIME
*
*     ==== External functions ==========================================
      DOUBLE PRECISION   DSECND, PZLANGE, PDLAMCH
      INTEGER            NUMROC, INDXG2P
      EXTERNAL           DSECND, PZLANGE, PDLAMCH, NUMROC, INDXG2P
*
*     ==== Intrinsic functions =========================================
...
*     Write table header to output
      IF( IAM.EQ.0 ) THEN
         WRITE( NOUT, FMT = 9999 ) 'PZGEQRF performance results'
         WRITE( NOUT, FMT = 9996 )
...
*                 Ask necessary size of working array for factorization
                  CALL PZGEQRF( M, N, A, 1, 1, DESCA, TAU, ASK_LWORK,-1,
     $                          INFO )
                  LWORK = INT(ASK_LWORK(1))
                  ALLOCATE( WORK( LWORK ) )
*                 Factor matrix A
                  TIME = DSECND()
                  CALL PZGEQRF( M, N, A, 1, 1, DESCA, TAU, WORK,
     $                          LWORK, INFO )
                  CALL BLACS_BARRIER( ICTXT, 'All' )
                  TIME = DSECND() - TIME
                  DEALLOCATE( WORK )
...
*                    Copy R and Q into different matrices
                     CALL PZLASET( 'L', DESCR( M_ ), DESCR( N_ ), ZERO,
     $                             ZERO, R, 1, 1, DESCR )
                     CALL PZLACPY( 'U', DESCA( M_ ), DESCA( N_ ), A, 1,
     $                             1, DESCA, R, 1, 1, DESCR )
                     CALL PZLACPY( 'L', DESCA( M_ ), DESCA( N_ ), A, 1,
     $                             1, DESCA, Q, 1, 1, DESCQ )
*
*                    Generate matrix Q from reflections                    
                     CALL PZUNGQR( M, MIN( M, N ), MIN( M, N ), Q, 1, 1,
...
*                    Calculate norm of the matrix A ( 1 or infinity)
                     ALLOCATE( WORK( LWORK ) )
                     ANORM = PZLANGE( 'I', M, N, A, 1, 1, DESCA, WORK )
*
*                    Compute matrix Q*R-A -> A
                     CALL PZGEMM( 'N', 'N', M, N, MIN( M, N ), ONE,
...
*                    Calculate norm of the matrix Q*R-A ( 1 or infinity)
                     DIFNORM = PZLANGE( 'I', M, N, A, 1, 1, DESCA, WORK)
                     DEALLOCATE( WORK )      
*
