...
!
!      Performance and validation of PSPOTRF function
!
!*******************************************************************************
!
!===============================================================================
...
! INDXG2P
!
! In this example the matrix generated by PSMATGEN is factorized by PSPOTRF.
! For performance measurement only PSPOTRF execution time is taken into
! account.
!
!*******************************************************************************

      PROGRAM PSPOTRF_DRIVER
*     ==== Parameters ==================================================
...
*     ==== External subroutines ========================================
     ...
     $         PSPOTRF, DESCINIT, IGEBS2D, IGEBR2D, SGEBS2D, SGEBR2D, 
     $         PSLASET, PSLACPY, PSSYRK, CALC_AVERAGE_TIME
*
*     ==== External functions ==========================================
      DOUBLE PRECISION   DSECND
      REAL               PSLANSY, PSLAMCH
      INTEGER            NUMROC, INDXG2P, ICEIL, ILCM
      EXTERNAL           DSECND, PSLANSY, PSLAMCH, NUMROC, INDXG2P,
     $                   ICEIL, ILCM
*
*     ==== Intrinsic functions =========================================
...
*     Write table header to output
      IF( IAM.EQ.0 ) THEN
         WRITE( NOUT, FMT = 9999 ) 'PSPOTRF performance results'
         WRITE( NOUT, FMT = 9996 )
...
*                 Factor matrix A
                  TIME = DSECND()
                  CALL PSPOTRF( UPLO, N, A, 1, 1, DESCA, INFO )
                  CALL BLACS_BARRIER( ICTXT, 'All' )
                  TIME = DSECND() - TIME
*
                  TIME_ITER( LNCH ) = TIME
...
*                    Copy UPLO part of factorized matrix into a zero matrix
                     CALL PSLASET( 'All', N, N, ZERO, ZERO, A0, 1, 1,
     $                             DESCA0 )
                     CALL PSLACPY( UPLO, N, N, A, 1, 1, DESCA, A0, 1, 1,
     $                             DESCA0 )
*
*                    Regenerate matrix A
                     CALL PSMATGEN( ICTXT, 'Symm', 'Diag', DESCA( M_ ),
...
*                    Calculate norm of the matrix A ( 1 or infinity)
                     ALLOCATE( WORK( LWORK ) )      
                     ANORM = PSLANSY( 'I', UPLO, N, A, 1, 1, DESCA,
     $                                WORK )
*
...
*              Floating point operations per second
               IF( TIME.GT.0 ) THEN
                  GFLOPS = ( ( DBLE( N )**3 )/3.0D+0 )/TIME*1.0D-9
               ELSE
                  PRINT *,'Invalid timer'
                  IF( IAM.EQ.0 ) THEN
                     STOP 3
