...
!
!      Performance and validation of PZGETRF function
!
!*******************************************************************************
!
!===============================================================================
...
! INDXG2P
!
! In this example the matrix generated by PZMATGEN is factorized by PZGETRF.
! For performance measurement only PZGETRF execution time is taken into
! account.
!
!*******************************************************************************

      PROGRAM PZGETRF_DRIVER
*     ==== Parameters ==================================================
...
*     ==== External subroutines ========================================
     ...
     $         PZGETRF, DESCINIT, IGEBS2D, IGEBR2D, SGEBS2D, SGEBR2D, 
     $         PZLASET, PZLACPY, PZGEMM, PZLAPIV, DESCSET,
     $         CALC_AVERAGE_TIME
*
*     ==== External functions ==========================================
...
*     Write table header to output
      IF( IAM.EQ.0 ) THEN
         WRITE( NOUT, FMT = 9999 ) 'PZGETRF performance results'
         WRITE( NOUT, FMT = 9996 )
...
*                 Factor matrix A
                  TIME = DSECND()
                  CALL PZGETRF( M, N, A, 1, 1, DESCA, IPIV, INFO )
                  CALL BLACS_BARRIER( ICTXT, 'All' )
                  TIME = DSECND() - TIME
*
                  TIME_ITER( LNCH ) = TIME
...
*                    Copy U and L into different matrices
                     CALL PZLASET( 'L', DESCU( M_ ), DESCU( N_ ), ZERO,
     $                             ZERO, U, 1, 1, DESCU )
                     CALL PZLACPY( 'U', DESCA( M_ ), DESCA( N_ ), A, 1,
     $                             1, DESCA, U, 1, 1, DESCU )
                     CALL PZLACPY( 'L', DESCA( M_ ), DESCA( N_ ), A, 1,
     $                             1, DESCA, L, 1, 1, DESCL )
                     CALL PZLASET( 'U', DESCL( M_ ), DESCL( N_ ), ZERO,
     $                             ONE,  L, 1, 1, DESCL )
*
*                    Regenerate matrix A
                     CALL PZMATGEN( ICTXT, 'No', 'No', DESCA( M_ ),
...
*                    Compute P*L
     ...
     $                             RSRC_ ), NPROW ) + DESCA( MB_ ) )
                     CALL PZLAPIV( 'B', 'R', 'C', MIN( M, N ), N, L, 1,
...
*              Floating point operations per second
               IF( TIME.GT.0 ) THEN
                  GFLOPS = 4.0D+0 * ( ( DBLE( MIN( M, N ) )**2 * DBLE(
     $                     3*MAX( M, N ) - MIN( M, N ) ) ) / 3.0D+0 ) /
     $                     TIME*1.0D-9
               ELSE
                  PRINT *,'Invalid timer'
                  IF( IAM.EQ.0 ) THEN
