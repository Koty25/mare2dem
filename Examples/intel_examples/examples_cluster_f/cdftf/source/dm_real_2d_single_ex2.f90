...
!
!       Forward-Backward 2D real transform for single precision data.
!
!*****************************************************************************
...
!*****************************************************************************
      ...
      COMPLEX(4), ALLOCATABLE, TARGET :: LOCAL(:), WORK(:)
      REAL(4), POINTER :: X_IN(:,:)
      COMPLEX(4), POINTER :: X_OUT(:,:)
      INTEGER   ELEMENTSIZE
...
!
      STATUS = DftiCreateDescriptorDM(MKL_COMM,DESC,DFTI_SINGLE,        &
     &                                DFTI_REAL,2,LENGTHS)
...
!
      DO I = 1, M
         DO J = 1, NX
            X_IN(I,J) = INPUT_DATA_S(I,START_X+J-1)
         END DO
      END DO
...
!
      STATUS = DftiComputeBackwardDM(DESC,LOCAL,WORK)
...
!
      ...
      DO I = 1, M
         DO J = 1, NX
            MAXERR = MAX(MAXERR,                                        &
     &                   ABS(X_IN(I,J)-INPUT_DATA_S(I,START_X+J-1)))
         END DO
      END DO
      CALL MPI_ALLREDUCE(MAXERR, G_MAXERR, 1, MPI_REAL, MPI_MAX, COMM,  &
     &                   MPI_ERR)
      MAXERR = G_MAXERR
