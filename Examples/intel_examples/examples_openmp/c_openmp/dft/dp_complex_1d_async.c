...
/*
! A basic example of asynchronous DOUBLE-precision complex-to-complex in-place
...
int main(void)
{
    ...
    MKL_Complex16 *x1GPU = NULL;
    MKL_Complex16 *x2GPU = NULL;

    printf("Create DFTI descriptor\n");
    status = DftiCreateDescriptor(&descHandle, DFTI_DOUBLE, DFTI_COMPLEX, 1, N1);
    if (status != DFTI_NO_ERROR) goto failed;

    printf("Create GPU DFTI descriptor 1\n");
    status1GPU = DftiCreateDescriptor(&descHandle1GPU, DFTI_DOUBLE, DFTI_COMPLEX, 1, N1);
    if (status1GPU != DFTI_NO_ERROR) goto failed;

    printf("Create GPU DFTI descriptor 2\n");
    status2GPU = DftiCreateDescriptor(&descHandle2GPU, DFTI_DOUBLE, DFTI_COMPLEX, 1, N2);
...
#pragma omp target variant dispatch device(devNum)
    ...
    x = (MKL_Complex16 *)mkl_malloc(N1*sizeof(MKL_Complex16), 64);
    if (x == NULL) goto failed;

    printf("Allocate memory for GPU input array 1\n");
    x1GPU = (MKL_Complex16 *)mkl_malloc(N1*sizeof(MKL_Complex16), 64);
    if (x1GPU == NULL) goto failed;

    printf("Allocate memory for GPU input array 2\n");
    x2GPU = (MKL_Complex16 *)mkl_malloc(N2*sizeof(MKL_Complex16), 64);
