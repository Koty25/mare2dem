...
! multiple DOUBLE-precision complex-to-complex in-place 3D FFTs in a single call
...
int main(void)
{
    ...
    printf("Create DFTI descriptor for double-precision 3D FFT\n");
    status = DftiCreateDescriptor(&descHandle, DFTI_DOUBLE, DFTI_COMPLEX, 3, N);
    if (status != DFTI_NO_ERROR) goto failed;

    printf("Create GPU DFTI descriptor for double-precision 3D FFT\n");
...
#pragma omp target variant dispatch device(devNum)
    ...
    data = (MKL_Complex16*)mkl_malloc(numPoints * sizeof(MKL_Complex16), 64);
    if (data == NULL) goto failed;

    printf("Allocate GPU input array\n");
    dataGPU = (MKL_Complex16*)mkl_malloc(numPoints * sizeof(MKL_Complex16), 64);
