...
! 2D FFT on a (GPU) device using the OpenMP target (offload) interface of
! Intel(R) oneMKL DFTI
!******************************************************************************/
...
static void init(MKL_Complex8 *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG H1, MKL_LONG H2);
static int verify(MKL_Complex8 *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG H1, MKL_LONG H2);
...
#pragma omp target variant dispatch device(devNum)
    ...
    dataGPU = (MKL_Complex8*)mkl_malloc(N2*N1*sizeof(MKL_Complex8), 64);
...
#pragma omp target data map(tofrom:dataGPU[0:N2*N1]) device(devNum)
    {
#pragma omp target variant dispatch use_device_ptr(dataGPU) device(devNum) nowait
        {
