...
/*
! A basic example of SINGLE-precision complex-to-complex in-place 3D FFT on a
...
static void init(MKL_Complex8 *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG N3,
                 MKL_LONG H1, MKL_LONG H2, MKL_LONG H3);
static int verify(MKL_Complex8 *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG N3,
                  MKL_LONG H1, MKL_LONG H2, MKL_LONG H3);
...
int main(void)
{
    ...
    status = DftiCreateDescriptor(&descHandle, DFTI_SINGLE, DFTI_COMPLEX, 3, N);
    if (status != DFTI_NO_ERROR) goto failed;

    printf("Create GPU DFTI descriptor\n");
    statusGPU = DftiCreateDescriptor(&descHandleGPU, DFTI_SINGLE, DFTI_COMPLEX, 3, N);
...
#pragma omp target variant dispatch device(devNum)
    ...
    dataGPU = (MKL_Complex8*)mkl_malloc(N3*N2*N1*sizeof(MKL_Complex8), 64);
...
float TWOPI = 6.2831853071795864769f;
    ...
    for (MKL_LONG n3 = 0; n3 < N3; n3++) {
        for (MKL_LONG n2 = 0; n2 < N2; n2++) {
            for (MKL_LONG n1 = 0; n1 < N1; n1++) {
                ...
                data[index].real = cosf(phase) / (N3*N2*N1);
                data[index].imag = sinf(phase) / (N3*N2*N1);
            }
        }
    }
}
                  ...
                  MKL_LONG H1, MKL_LONG H2, MKL_LONG H3)
{
    ...
    const float errthr = 5.0f * logf((float) N2*N1) / logf(2.0f) * FLT_EPSILON;
    ...
    for (MKL_LONG n3 = 0; n3 < N3; ++n3) {
        for (MKL_LONG n2 = 0; n2 < N2; ++n2) {
            for (MKL_LONG n1 = 0; n1 < N1; ++n1) {
                ...
                if ((n1-H1)%N1==0 && (n2-H2)%N2==0 && (n3-H3)%N3==0) re_exp = 1.0f;
