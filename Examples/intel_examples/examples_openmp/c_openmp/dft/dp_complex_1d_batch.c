...
! multiple DOUBLE-precision complex-to-complex in-place 1D FFTs in a single call
...
static void init(MKL_Complex16 *x, MKL_LONG M, MKL_LONG N, MKL_LONG H);
static int verify(MKL_Complex16 *x, MKL_LONG M, MKL_LONG N, MKL_LONG H);
...
int main(void)
{
    ...
    status = DftiSetValue(descHandleGPU, DFTI_NUMBER_OF_TRANSFORMS, M);
    if (status != DFTI_NO_ERROR) goto failed;
    ...
    status = DftiSetValue(descHandleGPU, DFTI_INPUT_DISTANCE, N);
    if (status != DFTI_NO_ERROR) goto failed;
...
#pragma omp target variant dispatch device(devNum)
    ...
    x = (MKL_Complex16 *)mkl_malloc(numPoints*sizeof(MKL_Complex16), 64);
    if (x == NULL) goto failed;

    printf("Allocate memory for GPU input array\n");
    xGPU = (MKL_Complex16 *)mkl_malloc(numPoints*sizeof(MKL_Complex16), 64);
...
#pragma omp target data map(tofrom:xGPU[0:numPoints]) device(devNum)
    {
...
static void init(MKL_Complex16 *data, MKL_LONG M, MKL_LONG N, MKL_LONG H)
{
    for (MKL_LONG m = 0; m < M; ++m) {
        for (MKL_LONG n = 0; n < N; ++n) {
            double phase =  moda(n, H, N) / N;
            MKL_LONG mn = m*N  + n;
            data[mn].real = cos(TWOPI * phase) / N;
            data[mn].imag = sin(TWOPI * phase) / N;
        }
    }
}

// Verify that for all m, data(m,.) has unit peak at H
static int verify(MKL_Complex16 *data, MKL_LONG M, MKL_LONG N, MKL_LONG H)
{
