...
! multiple DOUBLE-precision complex-to-complex in-place 2D FFTs in a single call
...
int main(void)
{
    ...
    printf("Create DFTI descriptor for double-precision 2D FFT\n");
    status = DftiCreateDescriptor(&descHandle, DFTI_DOUBLE, DFTI_COMPLEX, 2, N);
    if (status != DFTI_NO_ERROR) goto failed;

    printf("Create GPU DFTI descriptor for double-precision 2D FFT\n");
