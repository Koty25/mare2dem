...
/*
! An example of SINGLE-precision real-to-complex out-of-place 2D FFT on a (GPU)
...
static void init_r(float *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG H1, MKL_LONG H2);
static void init_c(MKL_Complex8 *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG H1, MKL_LONG H2);
static int verify_c(MKL_Complex8 *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG H1, MKL_LONG H2);
static int verify_r(float *data, MKL_LONG N1, MKL_LONG N2, MKL_LONG H1, MKL_LONG H2);
...
int main(void)
{
    ...
    float        *dataGPU_real  = NULL;
    MKL_Complex8 *data_cmplx    = NULL;
    MKL_Complex8 *dataGPU_cmplx = NULL;
    ...
    statusGPU = DftiCreateDescriptor(&descHandleGPU, DFTI_SINGLE, DFTI_REAL, 2, N);
...
#pragma omp target variant dispatch device(devNum)
    ...
    data_real = (float*)mkl_malloc(numRealPts*sizeof(float), 64);
    data_cmplx = (MKL_Complex8*)mkl_malloc(numCmplxPts*sizeof(MKL_Complex8), 64);
    if (data_real == NULL || data_cmplx == NULL) goto failed;

    printf("Allocate GPU data arrays\n");
    dataGPU_real = (float*)mkl_malloc(numRealPts*sizeof(float), 64);
    dataGPU_cmplx = (MKL_Complex8*)mkl_malloc(numCmplxPts*sizeof(MKL_Complex8), 64);
