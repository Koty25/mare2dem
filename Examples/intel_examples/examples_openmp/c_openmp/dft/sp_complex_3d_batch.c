...
! multiple SINGLE-precision complex-to-complex in-place 3D FFTs in a single call
...
int main(void)
{
    ...
    const MKL_LONG inputDistance = N[0]*N[1]*N[2];

    // Number of transforms
    MKL_LONG M = 3;

    const MKL_LONG numPoints = M * N[0]*N[1]*N[2];
    ...
    printf("Create DFTI descriptor for single-precision 3D FFT\n");
    status = DftiCreateDescriptor(&descHandle, DFTI_SINGLE, DFTI_COMPLEX, 3, N);
    if (status != DFTI_NO_ERROR) goto failed;

    printf("Create GPU DFTI descriptor for single-precision 3D FFT\n");
...
#pragma omp target variant dispatch device(devNum)
    ...
    init(dataGPU, M, N[2], N[1], N[0], H[2], H[1], H[0]);
...
#pragma omp target variant dispatch use_device_ptr(dataGPU) device(devNum)
    ...
    statusGPU = verify(dataGPU, M, N[2], N[1], N[0], H[2], H[1], H[0]);
