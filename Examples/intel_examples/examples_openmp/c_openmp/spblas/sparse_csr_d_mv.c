...
*             offload example for mkl_sparse_d_mv
*
********************************************************************************
*
...
*
*       A*x = y using mkl_sparse_d_mv omp offload
...
#define NNZ 13
    ...
    double values[NNZ]    = { 1.0, -1.0,     -3.0,
                             ...
                             -4.0,       2.0, 7.0,
                                    8.0,          -5.0 };
    MKL_INT colIndex[NNZ] = { 0,      1,        3,
                              0,      1,
                                           2,   3,   4,
                              0,           2,   3,
                                      1,             4 };
    MKL_INT rowStart[N + 1] = {0, 3, 5, 8, 11, 13};
    ...
    printf("\n EXAMPLE PROGRAM FOR mkl_sparse_d_mv omp_offload \n");
    printf("---------------------------------------------------\n");
    printf("\n");
    printf("   INPUT DATA FOR mkl_sparse_d_mv omp offload    \n");
...
#pragma omp target variant dispatch device(devNum) use_device_ptr(ia, ja, a)
        {
            status_gpu1 = mkl_sparse_d_create_csr(&csrA_gpu, SPARSE_INDEX_BASE_ZERO, N, M, ia,
                                                  ia + 1, ja, a);
        }
...
#pragma omp target variant dispatch device(devNum)
    ...
    printf("   OUTPUT DATA FOR mkl_sparse_d_mv_omp_offload \n");
