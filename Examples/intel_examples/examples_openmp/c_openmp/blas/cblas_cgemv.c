...
/*
!  Content:
!      oneMKL CBLAS_CGEMV OpenMP offload Example Program Text 
!******************************************************************************/
...
#pragma omp target map(y[0:sizey], y_ref[0:sizey])
    ...
    cblas_cgemv(CblasColMajor, CblasNoTrans, m, n, &alpha, a, lda, x, incx, &beta, y_ref, incy);
...
#pragma omp target variant dispatch device(dnum) use_device_ptr(a, x, y)
        {
            cblas_cgemv(CblasColMajor, CblasNoTrans, m, n, &alpha, a, lda, x, incx, &beta, y, incy);
        }
