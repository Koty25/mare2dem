...
/*
!  Content:
!      oneMKL CBLAS_ZGEMV OpenMP offload Example Program Text 
!******************************************************************************/
...
int main() {
    ...
    MKL_Complex16 *a, *x, *y, *y_ref, alpha, beta;
    ...
    // allocate matrices
    a = (MKL_Complex16 *)mkl_malloc(sizea * sizeof(MKL_Complex16), 64);
    x = (MKL_Complex16 *)mkl_malloc(sizex * sizeof(MKL_Complex16), 64);
    y = (MKL_Complex16 *)mkl_malloc(sizey * sizeof(MKL_Complex16), 64);
    y_ref = (MKL_Complex16 *)mkl_malloc(sizey * sizeof(MKL_Complex16), 64);
    ...
    // initialize matrices
    init_double_complex_array(sizea, a, 1);
    init_double_complex_array(sizex, x, 1);
...
#pragma omp target map(y[0:sizey], y_ref[0:sizey])
    ...
    cblas_zgemv(CblasColMajor, CblasNoTrans, m, n, &alpha, a, lda, x, incx, &beta, y_ref, incy);
...
#pragma omp target variant dispatch device(dnum) use_device_ptr(a, x, y)
        {
            cblas_zgemv(CblasColMajor, CblasNoTrans, m, n, &alpha, a, lda, x, incx, &beta, y, incy);
        }
