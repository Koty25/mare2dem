...
!      FORTRAN OpenMP offload examples for SGER
!*******************************************************************************

include "mkl_omp_offload.f90"
include "common_blas.f90"

program sger_example
...
! initialize matrices
call sinit_matrix('N', m, n, lda, a)
call sinit_vector(m, incx, x)
call sinit_vector(n, incy, y)
call scopy_matrix('N', m, n, lda, a, a_ref)

! Calling sger on the CPU
call sger(m, n, alpha, x, incx, y, incy, a_ref, lda)

! Calling sger on the GPU
...
passed = scheck_matrix(m, n, lda, a, a_ref)
...
call sprint_matrix(m, n, lda, a)

print *, "Matrix computed on CPU:"
call sprint_matrix(m, n, lda, a_ref)
