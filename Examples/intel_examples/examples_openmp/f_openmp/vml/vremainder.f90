...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3EEEF750', & ! 0.466730595    
    ...
    do a = 1, 3
        ...
        call vsremainder(VLEN, varg1, varg2, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,varg2,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,varg2,vmres1)
        call vmsremainder(VLEN, varg1, varg2, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FDDDEE74E3BBDF0', & ! 0.466729952239773205     
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,varg2,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,varg2,vres1)
        call vdremainder(VLEN, varg1, varg2, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,varg2,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,varg2,vmres1)
        call vmdremainder(VLEN, varg1, varg2, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program remainder_example
    ...
    character (len = *), parameter :: funcname = "remainder"
