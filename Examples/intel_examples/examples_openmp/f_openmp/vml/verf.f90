...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3F29FF1C', & ! 0.66404891     
                     Z'BE707D97', & ! -0.234854087   
                     Z'3F13A33B', & ! 0.576709449    
                     Z'3F19F50C'  / ! 0.601395369    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FE53FE388571266', & ! 0.664048925675683632     
                     Z'BFCE0FB2D80C1EAA', & ! -0.234854083530298852    
                     Z'3FE2746759551790', & ! 0.576709436871839287     
                     Z'3FE33EA175373E87'  / ! 0.601395348488907966     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vderf(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmderf(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program erf_example
    ...
    character (len = *), parameter :: funcname = "erf"
