...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3F0719BD', & ! 0.527736485    
                     Z'BF4784FB', & ! -0.779372871   
                     Z'3F0FA1B9', & ! 0.561061442    
                     Z'3F0D2095'  / ! 0.551278412    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FE0E337A3487F51', & ! 0.527736491119847995     
                     Z'BFE8F09F674D7307', & ! -0.779372884524122678    
                     Z'3FE1F4372CC88437', & ! 0.561061465709037033     
                     Z'3FE1A412983CABD1'  / ! 0.551278397882351112     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdinvcbrt(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdinvcbrt(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program invcbrt_example
    ...
    character (len = *), parameter :: funcname = "invcbrt"
