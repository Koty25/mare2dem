...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3EFEA7D8', & ! 0.497374296    
                     Z'BF5B5EAB', & ! -0.856913269   
                     Z'BF14FEBF', & ! -0.582012117   
                     Z'BE9E5396'  / ! -0.30923146    
    data test_ref2 / Z'3F5E16D8', & ! 0.867536068    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FDFD4F93E99B2E0', & ! 0.497373877652348639     
                     Z'BFEB6BD5549D70BC', & ! -0.85691324735991925     
                     Z'BFE29FD7C840E7D0', & ! -0.582012072677793313    
                     Z'BFD3CA723281D19B'  / ! -0.309231328318924248    
    data test_ref2 / Z'3FEBC2DB7AB89950', & ! 0.867536296548488295     
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1,vres2)
        !$omp target variant dispatch use_device_ptr(varg1,vres1,vres2)
        call vdsincos(VLEN, varg1, vres1, vres2)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1,vmres2)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1,vmres2)
        call vmdsincos(VLEN, varg1, vmres1, vmres2, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program sincos_example
    ...
    character (len = *), parameter :: funcname = "sincos"
