...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'40109A93', & ! 2.25943446     
                     Z'3FDCBE20', & ! 1.72455215     
                     Z'400D1D83', & ! 2.20492625     
                     Z'400E1251'  / ! 2.21986794     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'400213525A3C64F9', & ! 2.25943441864785344      
                     Z'3FFB97C3FFEB275C', & ! 1.72455215423766273      
                     Z'4001A3B0660BB464', & ! 2.20492629740924606      
                     Z'4001C24A31E8EF35'  / ! 2.21986807815656073      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdlog1p(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdlog1p(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program log1p_example
    ...
    character (len = *), parameter :: funcname = "log1p"
