...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'BF53F756', & ! -0.827992797   
                     Z'BF1B3F64', & ! -0.606436014   
                     Z'3EFBEB1E', & ! 0.492028177    
                     Z'3FC433EC'  / ! 1.53283453     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'BFEA7EEE13BC9A32', & ! -0.827994383381172705    
                     Z'BFE367ED418AB69C', & ! -0.606436374675201062    
                     Z'3FDF7D60444F3889', & ! 0.492027346332982563     
                     Z'3FF88679C577E974'  / ! 1.53283097396942214      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdtand(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdtand(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program tand_example
    ...
    character (len = *), parameter :: funcname = "tand"
    
    total_errs = 0

    data FLOAT_MAXULP /512.0,512.0,5000.0/
