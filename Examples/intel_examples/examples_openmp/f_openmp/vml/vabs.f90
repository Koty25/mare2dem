...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'40D9B85C', & ! 6.80375481     
                     Z'4007309A', & ! 2.1123414      
                     Z'40B52EFA', & ! 5.66198444     
                     Z'40BF006A'  / ! 5.96880054     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'401B370B60E66E18', & ! 6.80375434309419092      
                     Z'4000E6134801CC26', & ! 2.11234146361813924      
                     Z'4016A5DF421D4BBE', & ! 5.66198447517211711      
                     Z'4017E00D485FC01A'  / ! 5.96880066952146571      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdabs(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdabs(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_float_complex(funcname)
    ...
    data test_ref1 / Z'40E3F8C8', & ! 7.1241188      
                     Z'4103A212', & ! 8.22706795     
                     Z'412375B5', & ! 10.2162371     
                     Z'40C978BB'  / ! 6.29598761     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vcabs(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmcabs(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
          ...
          errs = errs + check_result_float_complex(VML_ARG1C_RES1R, varg1(i), varg1(i), & 
                           cres1, cres1, cref1, cref1, "v"//funcname, a)
          errs = errs + check_result_float_complex(VML_ARG1C_RES1R, varg1(i), varg1(i), & 
                           cmres1, cmres1, cref1, cref1, "vm"//funcname, a)
        enddo
    enddo
...
integer (kind=4) function test_double_complex(funcname)
    ...
    data test_ref1 / Z'401C7F18D50375CE', & ! 7.12411816438311796      
                     Z'402074424557113D', & ! 8.22706810653527931      
                     Z'40246EB68D8FA14C', & ! 10.2162365186528845      
                     Z'40192F176CA35836'  / ! 6.29598779437042388      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vzabs(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmzabs(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
          ...
          errs  = errs + check_result_double_complex(VML_ARG1C_RES1R, varg1(i), varg1(i), & 
                            cres1, cres1, cref1, cref1, "v"//funcname, a)
          errs  = errs + check_result_double_complex(VML_ARG1C_RES1R, varg1(i), varg1(i), & 
                            cmres1, cmres1, cref1, cref1, "vm"//funcname, a)
        enddo
...
program abs_example
    ...
    character (len = *), parameter :: funcname = "abs"
