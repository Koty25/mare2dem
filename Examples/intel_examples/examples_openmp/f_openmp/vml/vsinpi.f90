...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3F140501', & ! 0.578201354    
                     Z'BEB0F8FB', & ! -0.34564957    
                     Z'BF5F8FB0', & ! -0.873286247   
                     Z'BDC86A5C'  / ! -0.0978591144  
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FE280A29F52D2F1', & ! 0.578202544382831474     
                     Z'BFD61F2018124760', & ! -0.345649741656520959    
                     Z'BFEBF1F5F0704FD0', & ! -0.873286218268271242    
                     Z'BFB90D44ED33CC3C'  / ! -0.0978587226016295886   
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdsinpi(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdsinpi(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program sinpi_example
    ...
    character (len = *), parameter :: funcname = "sinpi"
