...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3F4DC2E0', & ! 0.803754807    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FE9B85B073370C0', & ! 0.803754343094190915     
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdfrac(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdfrac(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program frac_example
    ...
    character (len = *), parameter :: funcname = "frac"
