...
integer (kind=4) function test_float(funcname)
    ...
    real      (kind=4),allocatable :: varg1(:), vres1(:), vmres1(:), vref1(:), vres2(:), vmres2(:), vref2(:)
    integer   (kind=4) i, a, errs
    integer   (kind=4) VLEN
    parameter (VLEN = 4)
    integer   (kind=4) test_arg1(VLEN)
    integer   (kind=4) test_ref1(VLEN)
    integer   (kind=4) test_ref2(VLEN)
    ...
    data test_ref1 / Z'40C00000', & ! 6              
                     Z'C0000000', & ! -2             
                     Z'40A00000', & ! 5              
                     Z'40A00000'  / ! 5              
    data test_ref2 / Z'3F4DC2E0', & ! 0.803754807    
                     Z'BDE61340', & ! -0.112341404   
                     Z'3F2977D0', & ! 0.661984444    
                     Z'3F780350'  / ! 0.968800545    
    ...
    do i = 1, VLEN
        ...
        vref2(i) = as_float(test_ref2(i))
        vres2(i) = as_float(nan_value)
        vmres2(i) = as_float(nan_value)
    enddo
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1,vres2)
        !$omp target variant dispatch use_device_ptr(varg1,vres1,vres2)
        call vsmodf(VLEN, varg1, vres1, vres2)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1,vmres2)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1,vmres2)
        call vmsmodf(VLEN, varg1, vmres1, vmres2, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
          errs = errs + check_result_float(VML_ARG1_RES2, varg1(i), varg1(i), & 
                           vres1(i), vres2(i), vref1(i), vref2(i), "v"//funcname, a)
          errs = errs + check_result_float(VML_ARG1_RES2, varg1(i), varg1(i), & 
                           vmres1(i), vmres2(i), vref1(i), vref2(i), "vm"//funcname, a)
        enddo
    enddo
...
integer (kind=4) function test_double(funcname)
    ...
    real      (kind=8),allocatable :: varg1(:), vres1(:), vmres1(:), vref1(:), vres2(:), vmres2(:), vref2(:)
    integer   (kind=4) i, a, errs
    integer   (kind=8) VLEN
    parameter (VLEN = 4)
    integer   (kind=8) test_arg1(VLEN)
    integer   (kind=8) test_ref1(VLEN)
    integer   (kind=8) test_ref2(VLEN)
    ...
    data test_ref1 / Z'4018000000000000', & ! 6                        
                     Z'C000000000000000', & ! -2                       
                     Z'4014000000000000', & ! 5                        
                     Z'4014000000000000'  / ! 5                        
    data test_ref2 / Z'3FE9B85B073370C0', & ! 0.803754343094190915     
                     Z'BFBCC269003984C0', & ! -0.112341463618139237    
                     Z'3FE52EFA10EA5DF0', & ! 0.661984475172117115     
                     Z'3FEF006A42FE00D0'  / ! 0.968800669521465707     
    ! Arguments and reference results end
    ...
    do i = 1, VLEN
        ...
        vref2(i) = as_double(test_ref2(i))
        vres2(i) = as_double(nan_value)
        vmres2(i) = as_double(nan_value)
    enddo
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1,vres2)
        !$omp target variant dispatch use_device_ptr(varg1,vres1,vres2)
        call vdmodf(VLEN, varg1, vres1, vres2)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1,vmres2)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1,vmres2)
        call vmdmodf(VLEN, varg1, vmres1, vmres2, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
          errs = errs + check_result_double(VML_ARG1_RES2, varg1(i), varg1(i), & 
                            vres1(i), vres2(i), vref1(i), vref2(i), "v"//funcname, a)
          errs = errs + check_result_double(VML_ARG1_RES2, varg1(i), varg1(i), & 
                            vmres1(i), vmres2(i), vref1(i), vref2(i), "vm"//funcname, a)
        enddo
...
program modf_example
    ...
    character (len = *), parameter :: funcname = "modf"
