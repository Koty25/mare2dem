...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3E168150', & ! 0.146977663    
                     Z'BEF26293', & ! -0.473408312   
                     Z'3E34DAF6', & ! 0.17661652     
                     Z'3E2B8F0B'  / ! 0.167537853    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FC2D02A1B61C64B', & ! 0.146977675790866819     
                     Z'BFDE4C525D09E911', & ! -0.473408308847539649    
                     Z'3FC69B5EC2D437AE', & ! 0.176616521006903893     
                     Z'3FC571E14E0D800C'  / ! 0.167537844764411026     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdinv(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdinv(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program inv_example
    ...
    character (len = *), parameter :: funcname = "inv"
