...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3E860CD7', & ! 0.26181671     
                     Z'3F1157F3', & ! 0.567748249    
                     Z'3E9DE862', & ! 0.308413565    
                     Z'3E97C2A2'  / ! 0.296406806    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FD0C19AFA2583F6', & ! 0.261816734584555788     
                     Z'3FE22AFE6445C016', & ! 0.567748256535199003     
                     Z'3FD3BD0C52F35C07', & ! 0.308413582807986975     
                     Z'3FD2F85444FAAB98'  / ! 0.296406810152512801     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdacospi(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdacospi(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program acospi_example
    ...
    character (len = *), parameter :: funcname = "acospi"
