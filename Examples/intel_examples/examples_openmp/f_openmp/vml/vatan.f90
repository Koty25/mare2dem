...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3FB661ED', & ! 1.42486346     
                     Z'BF907785', & ! -1.12864745    
                     Z'3FB2AF8F', & ! 1.39598262     
                     Z'3FB3D07E'  / ! 1.40480018     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FF6CC3DAB5A6D16', & ! 1.42486349997381501      
                     Z'BFF20EF0A5D69141', & ! -1.12864746838120333     
                     Z'3FF655F1DB631CA2', & ! 1.39598260591609646      
                     Z'3FF67A0FB53FB5DE'  / ! 1.40480013656939873      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdatan(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdatan(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_float_complex(funcname)
    ...
    data test_ref1 / Z'BFC3A3F8', Z'3E09DBCA', & ! -1.52844143     + i * 0.134627491    
                     Z'3FBDB99A', Z'3DAA6186', & ! 1.48222661      + i * 0.0831938237   
                     Z'BFC19B0F', Z'3DA15661', & ! -1.51254451     + i * 0.0787780359   
                     Z'3FB7BA3C', Z'BDA78E76'  / ! 1.43537092      + i * -0.0818146914  
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vcatan(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmcatan(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_double_complex(funcname)
    ...
    data test_ref1 / Z'BFF8747EFBF5F2BF', Z'3FC13B795AD816CE', & ! -1.52844141409074985      + i * 0.13462750373599025      
                     Z'3FF7B7334500B661', Z'3FB54C30C3922F24', & ! 1.48222662882053435       + i * 0.0831938245266284349    
                     Z'BFF83361D7717724', Z'3FB42ACC31C9F775', & ! -1.51254448087224436      + i * 0.0787780400805482978    
                     Z'3FF6F747899AD360', Z'BFB4F1CEA21834C6'  / ! 1.43537095786924596       + i * -0.0818146844614658642   
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vzatan(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmzatan(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program atan_example
    ...
    character (len = *), parameter :: funcname = "atan"
