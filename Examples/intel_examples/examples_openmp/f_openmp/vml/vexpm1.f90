...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'44610E64', & ! 900.224854     
                     Z'BF610921', & ! -0.879045546   
                     Z'438F5C09', & ! 286.719025     
                     Z'43C304A7'  / ! 390.036346     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'408C21CBAE86A8F2', & ! 900.224453975706865      
                     Z'BFEC21242FF4658C', & ! -0.87904557577273712     
                     Z'4071EB813828BC71', & ! 286.719047698140514      
                     Z'40786095200E2E6A'  / ! 390.036407523532603      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdexpm1(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdexpm1(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program expm1_example
    ...
    character (len = *), parameter :: funcname = "expm1"
