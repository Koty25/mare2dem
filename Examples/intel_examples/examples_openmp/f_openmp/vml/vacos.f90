...
integer (kind=4) function test_float(funcname)
    ...
    data test_arg1 / Z'3F2E2D16', & ! 0.680375457    
                     Z'BE584DC4', & ! -0.211234152   
                     Z'3F10F262', & ! 0.566198468    
                     Z'3F18CD22'  / ! 0.596880078    
    data test_ref1 / Z'3F5290C5', & ! 0.822521508    
                     Z'3FE44E1C', & ! 1.78363371     
                     Z'3F780A79', & ! 0.9689098      
                     Z'3F6E626E'  / ! 0.931189418    
...
integer (kind=4) function test_double(funcname)
    ...
    ! Arguments and reference results begin
    data test_arg1 / Z'3FE5C5A2B3EB8B46', & ! 0.680375434309419047     
                     Z'BFCB09B873361370', & ! -0.211234146361813924    
                     Z'3FE21E4C34E43C98', & ! 0.566198447517211711     
                     Z'3FE319A439E63348'  / ! 0.596880066952146571     
    data test_ref1 / Z'3FEA5218ABED936B', & ! 0.822521529957709219     
                     Z'3FFC89C38B81B281', & ! 1.78363375181939454      
                     Z'3FEF014F38AD46CB', & ! 0.968909846016879128     
                     Z'3FEDCC4DD5287A83'  / ! 0.931189457249118724     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdacos(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdacos(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_float_complex(funcname)
    ...
    ! Arguments and reference results begin
    data test_arg1 / Z'BE584DC4', Z'3F2E2D16', & ! -0.211234152    + i * 0.680375457    
                     Z'3F18CD22', Z'3F10F262', & ! 0.596880078     + i * 0.566198468    
                     Z'BF1ADA8C', Z'3F52C372', & ! -0.604897261    + i * 0.823294759    
                     Z'3F095564', Z'BEA8BB5E'  / ! 0.536459208     + i * -0.329554498   
    data test_ref1 / Z'3FDF6BAE', Z'BF252AF2', & ! 1.74547362      + i * -0.645186543   
                     Z'3F8618E7', Z'BF1D4034', & ! 1.04763496      + i * -0.614260912   
                     Z'400210E5', Z'BF52C236', & ! 2.03228116      + i * -0.823275924   
                     Z'3F85D851', Z'3EBE9394'  / ! 1.04566395      + i * 0.372219682    
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vcacos(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmcacos(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_double_complex(funcname)
    ...
    ! Arguments and reference results begin
    data test_arg1 / Z'BFCB09B873361370', Z'3FE5C5A2B3EB8B46', & ! -0.211234146361813924     + i * 0.680375434309419047     
                     Z'3FE319A439E63348', Z'3FE21E4C34E43C98', & ! 0.596880066952146571      + i * 0.566198447517211711     
                     Z'BFE35B518066B6A3', Z'3FEA586E28F4B0DC', & ! -0.604897261413232079     + i * 0.823294715873568617     
                     Z'3FE12AAC76625558', Z'BFD5176BB5AA2ED8'  / ! 0.536459189623808008      + i * -0.32955448857022196     
    data test_ref1 / Z'3FFBED75C72580BA', Z'BFE4A55E2B892514', & ! 1.74547364989852705       + i * -0.645186505346972528    
                     Z'3FF0C31CDB3C5272', Z'BFE3A80667B42868', & ! 1.04763494147223613       + i * -0.614260866686220375    
                     Z'4000421CA734509A', Z'BFEA5846A28C515F', & ! 2.03228121403124096       + i * -0.823275868870535166    
                     Z'3FF0BB0A242AB476', Z'3FD7D272742FD332'  / ! 1.04566396835005326       + i * 0.372219670737922503     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vzacos(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmzacos(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program acos_example
    ...
    character (len = *), parameter :: funcname = "acos"
