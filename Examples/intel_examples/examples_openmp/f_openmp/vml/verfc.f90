...
integer (kind=4) function test_float(funcname)
    ...
    data test_arg1 / Z'4042A1D0', & ! 3.04112625     
                     Z'3EBB8B58', & ! 0.366297483    
                     Z'402CB5CA', & ! 2.69859552     
                     Z'403299DA'  / ! 2.79064035     
    data test_ref1 / Z'378EC21C', & ! 1.70181083e-05 
                     Z'3F1ABCBB', & ! 0.604442298    
                     Z'390DFF0B', & ! 0.000135418188 
                     Z'38A643FA'  / ! 7.92815845e-05 
...
integer (kind=4) function test_double(funcname)
    ...
    ! Arguments and reference results begin
    data test_arg1 / Z'4008543A06F0A874', & ! 3.04112630292825692      
                     Z'3FD7716B532EE2D8', & ! 0.366297560914558229     
                     Z'400596B927AB2D72', & ! 2.69859534255163513      
                     Z'4006533B2B6CA676'  / ! 2.79064020085643971      
    data test_ref1 / Z'3EF1D8431328C9AA', & ! 1.70181021507759823e-05  
                     Z'3FE357973F1784C6', & ! 0.604442237116153747     
                     Z'3F21BFE29CC4BFA2', & ! 0.000135418331760115091  
                     Z'3F14C8807F44637B'  / ! 7.92816570690645111e-05  
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vderfc(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmderfc(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program erfc_example
    ...
    character (len = *), parameter :: funcname = "erfc"
    
    total_errs = 0

    data FLOAT_MAXULP /256.0,256.0,256.0/
    data COMPLEX_FLOAT_MAXULP /FLOAT_COMPLEX_MAXULP_HA,FLOAT_COMPLEX_MAXULP_LA,FLOAT_COMPLEX_MAXULP_EP/
    data DOUBLE_MAXULP /1.0D10,1.0D10,1.0D10/
