...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'44614E64', & ! 901.224854     
                     Z'3DF7B6F5', & ! 0.120954432    
                     Z'438FDC09', & ! 287.719025     
                     Z'43C384A7'  / ! 391.036346     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'408C29CBAE86A8F2', & ! 901.224453975706865      
                     Z'3FBEF6DE805CD39C', & ! 0.120954424227262824     
                     Z'4071FB813828BC71', & ! 287.719047698140514      
                     Z'40787095200E2E6A'  / ! 391.036407523532603      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdexp(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdexp(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_float_complex(funcname)
    ...
    data test_ref1 / Z'3DD6E6C3', Z'3D7669F0', & ! 0.104932331     + i * 0.0601596236   
                     Z'439EFDD3', Z'C3639680', & ! 317.983002      + i * -227.587891    
                     Z'BA64E852', Z'3B0FB55F', & ! -0.000873212819 + i * 0.0021928174   
                     Z'C3532D29', Z'420314ED'  / ! -211.176407     + i * 32.7704353     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vcexp(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmcexp(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_double_complex(funcname)
    ...
    data test_ref1 / Z'3FBADCD8C17B13EC', Z'3FAECD3C3BF2B352', & ! 0.104932353245274335      + i * 0.0601595709971208953    
                     Z'4073DFBA8A9A56C2', Z'C06C72D028B4CC5F', & ! 317.983042338262862       + i * -227.58791003524945      
                     Z'BF4C9D04E55315EA', Z'3F61F6AC60E93AF9', & ! -0.000873210325743554007  + i * 0.00219281833350437301   
                     Z'C06A65A5A71D5FE7', Z'4040629C7263E09C'  / ! -211.176471288082411      + i * 32.7703993785555383      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vzexp(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmzexp(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program exp_example
    ...
    character (len = *), parameter :: funcname = "exp"
