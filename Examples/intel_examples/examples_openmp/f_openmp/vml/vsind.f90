...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'BF2343D1', & ! -0.637753546   
                     Z'3F04BECE', & ! 0.518536448    
                     Z'BEE209F3', & ! -0.441482157   
                     Z'BF566851'  / ! -0.837529242   
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'BFE4687BAF82E0D2', & ! -0.637754290385624367    
                     Z'3FE097DA3F612C50', & ! 0.518536685741660008     
                     Z'BFDC413BCAA36BE3', & ! -0.441481540574100773    
                     Z'BFEACD08F176F2A1'  / ! -0.837528678522705516    
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdsind(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdsind(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program sind_example
    ...
    character (len = *), parameter :: funcname = "sind"
    
    total_errs = 0

    data FLOAT_MAXULP /128.0,128.0,FLOAT_MAXULP_EP/
