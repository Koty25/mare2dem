...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'42DF7115', & ! 111.720863     
                     Z'3E6CD267', & ! 0.231271371    
                     Z'424A876A', & ! 50.6322403     
                     Z'427A85F2'  / ! 62.630806      
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'405BEE22060061FC', & ! 111.72082662618044       
                     Z'3FCD9A4CCC8509B5', & ! 0.231271362174153033     
                     Z'404950ED51D23267', & ! 50.6322424198486303      
                     Z'404F50BE6F64D25A'  / ! 62.6308116190236461      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdexp2(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdexp2(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program exp2_example
    ...
    character (len = *), parameter :: funcname = "exp2"
