...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'40D9B85B', & ! 6.80375433     
                     Z'40B52EFB', & ! 5.66198492     
                     Z'4103BA27', & ! 8.23294735     
                     Z'C052EA35'  / ! -3.29554486    
    ...
    do a = 1, 3
        ...
        call vsnextafter(VLEN, varg1, varg2, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,varg2,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,varg2,vmres1)
        call vmsnextafter(VLEN, varg1, varg2, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'401B370B60E66E17', & ! 6.80375434309419003      
                     Z'4016A5DF421D4BBF', & ! 5.661984475172118        
                     Z'40207744D998EE89', & ! 8.23294715873568528      
                     Z'C00A5D46A314BA8D'  / ! -3.29554488570221915     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,varg2,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,varg2,vres1)
        call vdnextafter(VLEN, varg1, varg2, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,varg2,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,varg2,vmres1)
        call vmdnextafter(VLEN, varg1, varg2, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program nextafter_example
    ...
    character (len = *), parameter :: funcname = "nextafter"
