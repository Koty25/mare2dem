...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'3EE83776', & ! 0.453548133    
                     Z'BEB7F0E1', & ! -0.359259635   
                     Z'3EE38282', & ! 0.444355071    
                     Z'3EE4F263'  / ! 0.447161764    
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'3FDD06EEC5825AEC', & ! 0.453548138504102694     
                     Z'BFD6FE1C2CE0567B', & ! -0.359259647202044341    
                     Z'3FDC7050395B41AB', & ! 0.444355064403704147     
                     Z'3FDC9E4C683DEF72'  / ! 0.447161771582378909     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdatanpi(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdatanpi(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program atanpi_example
    ...
    character (len = *), parameter :: funcname = "atanpi"
