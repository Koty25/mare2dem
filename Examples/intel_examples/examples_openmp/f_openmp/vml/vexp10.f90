...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'4AC23992', & ! 6364361        
                     Z'3BFCFE36', & ! 0.00772073399  
                     Z'48E035B2', & ! 459181.562     
                     Z'49633786'  / ! 930680.375     
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'415847308E0F2A6F', & ! 6364354.2196756443       
                     Z'3F7F9FC67EDE8ECC', & ! 0.00772073304497995425   
                     Z'411C06B6646C72F0', & ! 459181.598069950007      
                     Z'412C66F13C60E84B'  / ! 930680.617926844745      
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdexp10(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdexp10(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program exp10_example
    ...
    character (len = *), parameter :: funcname = "exp10"
