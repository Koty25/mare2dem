...
integer (kind=4) function test_float(funcname)
    ...
    data test_ref1 / Z'BF356B88', & ! -0.708672047   
                     Z'BEBC98D5', & ! -0.368353516   
                     Z'BFE56E6A', & ! -1.79243207    
                     Z'BDC961CD'  / ! -0.0983310714  
...
integer (kind=4) function test_double(funcname)
    ...
    data test_ref1 / Z'BFE6AD759D62B54F', & ! -0.708674247163495141    
                     Z'BFD7931B8E7CDD8B', & ! -0.368353737972449025    
                     Z'BFFCADCCD759EC08', & ! -1.79243167993149832     
                     Z'BFB92C330946EDEC'  / ! -0.0983306787156192352   
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdtanpi(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdtanpi(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program tanpi_example
    ...
    character (len = *), parameter :: funcname = "tanpi"
