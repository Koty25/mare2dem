...
integer (kind=4) function test_double(funcname)
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vdsin(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmdsin(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_float_complex(funcname)
    ...
    data test_ref1 / Z'C3C11171', Z'C36845CE', & ! -386.136261     + i * -232.272675    
                     Z'C231F219', Z'4308CE8E', & ! -44.4864235     + i * 136.806854     
                     Z'43DA5252', Z'44E4C2C8', & ! 436.643127      + i * 1830.08691     
                     Z'C12BD9EA', Z'C102E16D'  / ! -10.7407017     + i * -8.18003559    
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vcsin(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmcsin(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
integer (kind=4) function test_double_complex(funcname)
    ...
    data test_ref1 / Z'C078222D4F7FC3B9', Z'C06D08B90982015C', & ! -386.136062144356004      + i * -232.272587541500684     
                     Z'C0463E42A6B9A9E8', Z'406119D1D1680E9B', & ! -44.4864090353547112      + i * 136.806862548099929      
                     Z'407B4A4825964FE6', Z'409C9856E8CA5287', & ! 436.642613970905927       + i * 1830.08487239960391      
                     Z'C0257B3CA2F0A873', Z'C0205C2DC648E14A'  / ! -10.7406969946643809      + i * -8.18003673209809179     
    ! Arguments and reference results end
    ...
    do a = 1, 3
        ...
        !$omp target data map(varg1,vres1)
        !$omp target variant dispatch use_device_ptr(varg1,vres1)
        call vzsin(VLEN, varg1, vres1)
        !$omp end target variant dispatch
        !$omp end target data
		
        !$omp target data map(varg1,vmres1)
        !$omp target variant dispatch use_device_ptr(varg1,vmres1)
        call vmzsin(VLEN, varg1, vmres1, vml_accuracy_mode(a))
        !$omp end target variant dispatch
        !$omp end target data
        ! Check results
        do i = 1, VLEN
...
program sin_example
    ...
    character (len = *), parameter :: funcname = "sin"
