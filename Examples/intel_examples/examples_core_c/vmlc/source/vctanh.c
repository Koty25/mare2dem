...
int main()
{
  ...
  vcTanhI(vec_len,cA_I,inca,cBha0_I,incb);

  vmcTanh(vec_len,cA,cBep1,VML_EP);
  vmcTanhI(vec_len,cA_I,inca,cBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vcTanh(vec_len,cA,cBep2);
  vcTanhI(vec_len,cA_I,inca,cBep2_I,incb);

  vmcTanh(vec_len,cA,cBla1,VML_LA);
  vmcTanhI(vec_len,cA_I,inca,cBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vcTanh(vec_len,cA,cBla2);
  vcTanhI(vec_len,cA_I,inca,cBla2_I,incb);

  vmcTanh(vec_len,cA,cBha1,VML_HA);
  vmcTanhI(vec_len,cA_I,inca,cBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vcTanh(vec_len,cA,cBha2);
  vcTanhI(vec_len,cA_I,inca,cBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(cBha0_I[i*incb].real!=cBha1_I[i*incb].real || cBha0_I[i*incb].imag!=cBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vcTanhI and vmcTanhI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(cBla1_I[i*incb].real!=cBla2_I[i*incb].real || cBla1_I[i*incb].imag!=cBla2_I[i*incb].imag) {
      printf("Error! Difference between vcTanhI and vmcTanhI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(cBep1_I[i*incb].real!=cBep2_I[i*incb].real || cBep1_I[i*incb].imag!=cBep2_I[i*incb].imag) {
      printf("Error! Difference between vcTanhI and vmcTanhI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vcTanhI test/example program\n\n");
  printf("           Argument                           vcTanh\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
