...
int main()
{
  ...
  vsSinpiI(vec_len,fA_I,inca,fBha0_I,incb);

  vmsSinpi(vec_len,fA,fBep1,VML_EP);
  vmsSinpiI(vec_len,fA_I,inca,fBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vsSinpi(vec_len,fA,fBep2);
  vsSinpiI(vec_len,fA_I,inca,fBep2_I,incb);

  vmsSinpi(vec_len,fA,fBla1,VML_LA);
  vmsSinpiI(vec_len,fA_I,inca,fBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vsSinpi(vec_len,fA,fBla2);
  vsSinpiI(vec_len,fA_I,inca,fBla2_I,incb);

  vmsSinpi(vec_len,fA,fBha1,VML_HA);
  vmsSinpiI(vec_len,fA_I,inca,fBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vsSinpi(vec_len,fA,fBha2);
  vsSinpiI(vec_len,fA_I,inca,fBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(fBha0_I[i*incb]!=fBha1_I[i*incb] || fBha1_I[i*incb]!=fBha2_I[i*incb]) {
      printf("Error! Difference between vsSinpiI and vmsSinpiI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(fBla1_I[i*incb]!=fBla2_I[i*incb]) {
      printf("Error! Difference between vsSinpiI and vmsSinpiI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(fBep1_I[i*incb]!=fBep2_I[i*incb]) {
      printf("Error! Difference between vsSinpiI and vmsSinpiI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vsSinpiI test/example program\n\n");
  printf("           Argument                     vsSinpi\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
