...
int main()
{
  ...
  vcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBha0_I,incb);

  vmcMulByConj(vec_len,cA,cA,cBep1,VML_EP);
  vmcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vcMulByConj(vec_len,cA,cA,cBep2);
  vcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBep2_I,incb);

  vmcMulByConj(vec_len,cA,cA,cBla1,VML_LA);
  vmcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vcMulByConj(vec_len,cA,cA,cBla2);
  vcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBla2_I,incb);

  vmcMulByConj(vec_len,cA,cA,cBha1,VML_HA);
  vmcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vcMulByConj(vec_len,cA,cA,cBha2);
  vcMulByConjI(vec_len,cA_I,inca,cA_I,inca,cBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(cBha0_I[i*incb].real!=cBha1_I[i*incb].real || cBha0_I[i*incb].imag!=cBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vcMulByConjI and vmcMulByConjI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(cBla1_I[i*incb].real!=cBla2_I[i*incb].real || cBla1_I[i*incb].imag!=cBla2_I[i*incb].imag) {
      printf("Error! Difference between vcMulByConjI and vmcMulByConjI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(cBep1_I[i*incb].real!=cBep2_I[i*incb].real || cBep1_I[i*incb].imag!=cBep2_I[i*incb].imag) {
      printf("Error! Difference between vcMulByConjI and vmcMulByConjI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vcMulByConjI test/example program\n\n");
  printf("           Arguments                           vcMulByConj\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
