...
int main()
{
  ...
  vzCoshI(vec_len,zA_I,inca,zBha0_I,incb);

  vmzCosh(vec_len,zA,zBep1,VML_EP);
  vmzCoshI(vec_len,zA_I,inca,zBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vzCosh(vec_len,zA,zBep2);
  vzCoshI(vec_len,zA_I,inca,zBep2_I,incb);

  vmzCosh(vec_len,zA,zBla1,VML_LA);
  vmzCoshI(vec_len,zA_I,inca,zBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vzCosh(vec_len,zA,zBla2);
  vzCoshI(vec_len,zA_I,inca,zBla2_I,incb);

  vmzCosh(vec_len,zA,zBha1,VML_HA);
  vmzCoshI(vec_len,zA_I,inca,zBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vzCosh(vec_len,zA,zBha2);
  vzCoshI(vec_len,zA_I,inca,zBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(zBha0_I[i*incb].real!=zBha1_I[i*incb].real || zBha0_I[i*incb].imag!=zBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vzCoshI and vmzCoshI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(zBla1_I[i*incb].real!=zBla2_I[i*incb].real || zBla1_I[i*incb].imag!=zBla2_I[i*incb].imag) {
      printf("Error! Difference between vzCoshI and vmzCoshI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(zBep1_I[i*incb].real!=zBep2_I[i*incb].real || zBep1_I[i*incb].imag!=zBep2_I[i*incb].imag) {
      printf("Error! Difference between vzCoshI and vmzCoshI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vzCoshI test/example program\n\n");
  printf("           Argument                           vzCosh\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
