...
int main()
{
  ...
  vzSinhI(vec_len,zA_I,inca,zBha0_I,incb);

  vmzSinh(vec_len,zA,zBep1,VML_EP);
  vmzSinhI(vec_len,zA_I,inca,zBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vzSinh(vec_len,zA,zBep2);
  vzSinhI(vec_len,zA_I,inca,zBep2_I,incb);

  vmzSinh(vec_len,zA,zBla1,VML_LA);
  vmzSinhI(vec_len,zA_I,inca,zBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vzSinh(vec_len,zA,zBla2);
  vzSinhI(vec_len,zA_I,inca,zBla2_I,incb);

  vmzSinh(vec_len,zA,zBha1,VML_HA);
  vmzSinhI(vec_len,zA_I,inca,zBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vzSinh(vec_len,zA,zBha2);
  vzSinhI(vec_len,zA_I,inca,zBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(zBha0_I[i*incb].real!=zBha1_I[i*incb].real || zBha0_I[i*incb].imag!=zBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vzSinhI and vmzSinhI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(zBla1_I[i*incb].real!=zBla2_I[i*incb].real || zBla1_I[i*incb].imag!=zBla2_I[i*incb].imag) {
      printf("Error! Difference between vzSinhI and vmzSinhI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(zBep1_I[i*incb].real!=zBep2_I[i*incb].real || zBep1_I[i*incb].imag!=zBep2_I[i*incb].imag) {
      printf("Error! Difference between vzSinhI and vmzSinhI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vzSinhI test/example program\n\n");
  printf("           Argument                           vzSinh\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
