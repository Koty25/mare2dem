...
int main()
{
  ...
  vcConjI(vec_len,cA_I,inca,cBha0_I,incb);

  vmcConj(vec_len,cA,cBep1,VML_EP);
  vmcConjI(vec_len,cA_I,inca,cBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vcConj(vec_len,cA,cBep2);
  vcConjI(vec_len,cA_I,inca,cBep2_I,incb);

  vmcConj(vec_len,cA,cBla1,VML_LA);
  vmcConjI(vec_len,cA_I,inca,cBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vcConj(vec_len,cA,cBla2);
  vcConjI(vec_len,cA_I,inca,cBla2_I,incb);

  vmcConj(vec_len,cA,cBha1,VML_HA);
  vmcConjI(vec_len,cA_I,inca,cBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vcConj(vec_len,cA,cBha2);
  vcConjI(vec_len,cA_I,inca,cBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(cBha0_I[i*incb].real!=cBha1_I[i*incb].real || cBha0_I[i*incb].imag!=cBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vcConjI and vmcConjI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(cBla1_I[i*incb].real!=cBla2_I[i*incb].real || cBla1_I[i*incb].imag!=cBla2_I[i*incb].imag) {
      printf("Error! Difference between vcConjI and vmcConjI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(cBep1_I[i*incb].real!=cBep2_I[i*incb].real || cBep1_I[i*incb].imag!=cBep2_I[i*incb].imag) {
      printf("Error! Difference between vcConjI and vmcConjI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vcConjI test/example program\n\n");
  printf("           Argument                           vcConj\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
