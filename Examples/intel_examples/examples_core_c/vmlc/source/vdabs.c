...
int main()
{
  ...
  vdAbsI(vec_len,dA_I,inca,dBha0_I,incb);

  vmdAbs(vec_len,dA,dBep1,VML_EP);
  vmdAbsI(vec_len,dA_I,inca,dBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vdAbs(vec_len,dA,dBep2);
  vdAbsI(vec_len,dA_I,inca,dBep2_I,incb);

  vmdAbs(vec_len,dA,dBla1,VML_LA);
  vmdAbsI(vec_len,dA_I,inca,dBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vdAbs(vec_len,dA,dBla2);
  vdAbsI(vec_len,dA_I,inca,dBla2_I,incb);

  vmdAbs(vec_len,dA,dBha1,VML_HA);
  vmdAbsI(vec_len,dA_I,inca,dBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vdAbs(vec_len,dA,dBha2);
  vdAbsI(vec_len,dA_I,inca,dBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(dBha0_I[i*incb]!=dBha1_I[i*incb] || dBha1_I[i*incb]!=dBha2_I[i*incb]) {
      printf("Error! Difference between vdAbsI and vmdAbsI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(dBla1_I[i*incb]!=dBla2_I[i*incb]) {
      printf("Error! Difference between vdAbsI and vmdAbsI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(dBep1_I[i*incb]!=dBep2_I[i*incb]) {
      printf("Error! Difference between vdAbsI and vmdAbsI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vdAbsI test/example program\n\n");
  printf("           Argument                     vdAbs\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
