...
int main()
{
  ...
  vdSinI(vec_len,dA_I,inca,dBha0_I,incb);

  vmdSin(vec_len,dA,dBep1,VML_EP);
  vmdSinI(vec_len,dA_I,inca,dBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vdSin(vec_len,dA,dBep2);
  vdSinI(vec_len,dA_I,inca,dBep2_I,incb);

  vmdSin(vec_len,dA,dBla1,VML_LA);
  vmdSinI(vec_len,dA_I,inca,dBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vdSin(vec_len,dA,dBla2);
  vdSinI(vec_len,dA_I,inca,dBla2_I,incb);

  vmdSin(vec_len,dA,dBha1,VML_HA);
  vmdSinI(vec_len,dA_I,inca,dBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vdSin(vec_len,dA,dBha2);
  vdSinI(vec_len,dA_I,inca,dBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(dBha0_I[i*incb]!=dBha1_I[i*incb] || dBha1_I[i*incb]!=dBha2_I[i*incb]) {
      printf("Error! Difference between vdSinI and vmdSinI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(dBla1_I[i*incb]!=dBla2_I[i*incb]) {
      printf("Error! Difference between vdSinI and vmdSinI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(dBep1_I[i*incb]!=dBep2_I[i*incb]) {
      printf("Error! Difference between vdSinI and vmdSinI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vdSinI test/example program\n\n");
  printf("           Argument                     vdSin\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
