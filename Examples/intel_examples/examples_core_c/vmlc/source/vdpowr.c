...
int main()
{
  ...
  vdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBha0_I,incc);

  vmdPowr(vec_len,dA1,dA2,dBep1,VML_EP);
  vmdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBep1_I,incc,VML_EP);

  vmlSetMode(VML_EP);
  vdPowr(vec_len,dA1,dA2,dBep2);
  vdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBep2_I,incc);

  vmdPowr(vec_len,dA1,dA2,dBla1,VML_LA);
  vmdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBla1_I,incc,VML_LA);

  vmlSetMode(VML_LA);
  vdPowr(vec_len,dA1,dA2,dBla2);
  vdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBla2_I,incc);

  vmdPowr(vec_len,dA1,dA2,dBha1,VML_HA);
  vmdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBha1_I,incc,VML_HA);

  vmlSetMode(VML_HA);
  vdPowr(vec_len,dA1,dA2,dBha2);
  vdPowrI(vec_len,dA1_I,inca,dA2_I,incb,dBha2_I,incc);
  ...
  for(i=0;i<10;i++) {
    ...
    if(dBha0_I[i*incc]!=dBha1_I[i*incc] || dBha1_I[i*incc]!=dBha2_I[i*incc]) {
      printf("Error! Difference between vdPowrI and vmdPowrI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(dBla1_I[i*incc]!=dBla2_I[i*incc]) {
      printf("Error! Difference between vdPowrI and vmdPowrI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(dBep1_I[i*incc]!=dBep2_I[i*incc]) {
      printf("Error! Difference between vdPowrI and vmdPowrI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vdPowrI test/example program\n\n");
  printf("           Argument                     vdPowr\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
