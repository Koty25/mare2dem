...
int main()
{
  ...
  vsTandI(vec_len,fA_I,inca,fBha0_I,incb);

  vmsTand(vec_len,fA,fBep1,VML_EP);
  vmsTandI(vec_len,fA_I,inca,fBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vsTand(vec_len,fA,fBep2);
  vsTandI(vec_len,fA_I,inca,fBep2_I,incb);

  vmsTand(vec_len,fA,fBla1,VML_LA);
  vmsTandI(vec_len,fA_I,inca,fBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vsTand(vec_len,fA,fBla2);
  vsTandI(vec_len,fA_I,inca,fBla2_I,incb);

  vmsTand(vec_len,fA,fBha1,VML_HA);
  vmsTandI(vec_len,fA_I,inca,fBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vsTand(vec_len,fA,fBha2);
  vsTandI(vec_len,fA_I,inca,fBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(fBha0_I[i*incb]!=fBha1_I[i*incb] || fBha1_I[i*incb]!=fBha2_I[i*incb]) {
      printf("Error! Difference between vsTandI and vmsTandI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(fBla1_I[i*incb]!=fBla2_I[i*incb]) {
      printf("Error! Difference between vsTandI and vmsTandI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(fBep1_I[i*incb]!=fBep2_I[i*incb]) {
      printf("Error! Difference between vsTandI and vmsTandI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vsTandI test/example program\n\n");
  printf("           Argument                     vsTand\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
