...
int main()
{
  ...
  vsErfcInvI(vec_len,fA_I,inca,fBha0_I,incb);

  vmsErfcInv(vec_len,fA,fBep1,VML_EP);
  vmsErfcInvI(vec_len,fA_I,inca,fBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vsErfcInv(vec_len,fA,fBep2);
  vsErfcInvI(vec_len,fA_I,inca,fBep2_I,incb);

  vmsErfcInv(vec_len,fA,fBla1,VML_LA);
  vmsErfcInvI(vec_len,fA_I,inca,fBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vsErfcInv(vec_len,fA,fBla2);
  vsErfcInvI(vec_len,fA_I,inca,fBla2_I,incb);

  vmsErfcInv(vec_len,fA,fBha1,VML_HA);
  vmsErfcInvI(vec_len,fA_I,inca,fBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vsErfcInv(vec_len,fA,fBha2);
  vsErfcInvI(vec_len,fA_I,inca,fBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(fBha0_I[i*incb]!=fBha1_I[i*incb] || fBha1_I[i*incb]!=fBha2_I[i*incb]) {
      printf("Error! Difference between vsErfcInvI and vmsErfcInvI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(fBla1_I[i*incb]!=fBla2_I[i*incb]) {
      printf("Error! Difference between vsErfcInvI and vmsErfcInvI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(fBep1_I[i*incb]!=fBep2_I[i*incb]) {
      printf("Error! Difference between vsErfcInvI and vmsErfcInvI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vsErfcInvI test/example program\n\n");
  printf("           Argument                     vsErfcInv\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
