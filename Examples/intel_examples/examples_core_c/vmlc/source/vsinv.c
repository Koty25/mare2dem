...
int main()
{
  ...
  vsInvI(vec_len,fA_I,inca,fBha0_I,incb);

  vmsInv(vec_len,fA,fBep1,VML_EP);
  vmsInvI(vec_len,fA_I,inca,fBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vsInv(vec_len,fA,fBep2);
  vsInvI(vec_len,fA_I,inca,fBep2_I,incb);

  vmsInv(vec_len,fA,fBla1,VML_LA);
  vmsInvI(vec_len,fA_I,inca,fBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vsInv(vec_len,fA,fBla2);
  vsInvI(vec_len,fA_I,inca,fBla2_I,incb);

  vmsInv(vec_len,fA,fBha1,VML_HA);
  vmsInvI(vec_len,fA_I,inca,fBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vsInv(vec_len,fA,fBha2);
  vsInvI(vec_len,fA_I,inca,fBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(fBha0_I[i*incb]!=fBha1_I[i*incb] || fBha1_I[i*incb]!=fBha2_I[i*incb]) {
      printf("Error! Difference between vsInvI and vmsInvI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(fBla1_I[i*incb]!=fBla2_I[i*incb]) {
      printf("Error! Difference between vsInvI and vmsInvI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(fBep1_I[i*incb]!=fBep2_I[i*incb]) {
      printf("Error! Difference between vsInvI and vmsInvI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vsInvI test/example program\n\n");
  printf("           Argument                     vsInv\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
