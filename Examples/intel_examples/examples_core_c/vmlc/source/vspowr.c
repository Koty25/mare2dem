...
int main()
{
  ...
  vsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBha0_I,incc);

  vmsPowr(vec_len,fA1,fA2,fBep1,VML_EP);
  vmsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBep1_I,incc,VML_EP);

  vmlSetMode(VML_EP);
  vsPowr(vec_len,fA1,fA2,fBep2);
  vsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBep2_I,incc);

  vmsPowr(vec_len,fA1,fA2,fBla1,VML_LA);
  vmsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBla1_I,incc,VML_LA);

  vmlSetMode(VML_LA);
  vsPowr(vec_len,fA1,fA2,fBla2);
  vsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBla2_I,incc);

  vmsPowr(vec_len,fA1,fA2,fBha1,VML_HA);
  vmsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBha1_I,incc,VML_HA);

  vmlSetMode(VML_HA);
  vsPowr(vec_len,fA1,fA2,fBha2);
  vsPowrI(vec_len,fA1_I,inca,fA2_I,incb,fBha2_I,incc);
  ...
  for(i=0;i<10;i++) {
    ...
    if(fBha0_I[i*incc]!=fBha1_I[i*incc] || fBha1_I[i*incc]!=fBha2_I[i*incc]) {
      printf("Error! Difference between vsPowrI and vmsPowrI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(fBla1_I[i*incc]!=fBla2_I[i*incc]) {
      printf("Error! Difference between vsPowrI and vmsPowrI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(fBep1_I[i*incc]!=fBep2_I[i*incc]) {
      printf("Error! Difference between vsPowrI and vmsPowrI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vsPowrI test/example program\n\n");
  printf("           Argument                     vsPowr\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
