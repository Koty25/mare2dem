...
int main()
{
  ...
  vcTanI(vec_len,cA_I,inca,cBha0_I,incb);

  vmcTan(vec_len,cA,cBep1,VML_EP);
  vmcTanI(vec_len,cA_I,inca,cBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vcTan(vec_len,cA,cBep2);
  vcTanI(vec_len,cA_I,inca,cBep2_I,incb);

  vmcTan(vec_len,cA,cBla1,VML_LA);
  vmcTanI(vec_len,cA_I,inca,cBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vcTan(vec_len,cA,cBla2);
  vcTanI(vec_len,cA_I,inca,cBla2_I,incb);

  vmcTan(vec_len,cA,cBha1,VML_HA);
  vmcTanI(vec_len,cA_I,inca,cBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vcTan(vec_len,cA,cBha2);
  vcTanI(vec_len,cA_I,inca,cBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(cBha0_I[i*incb].real!=cBha1_I[i*incb].real || cBha0_I[i*incb].imag!=cBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vcTanI and vmcTanI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(cBla1_I[i*incb].real!=cBla2_I[i*incb].real || cBla1_I[i*incb].imag!=cBla2_I[i*incb].imag) {
      printf("Error! Difference between vcTanI and vmcTanI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(cBep1_I[i*incb].real!=cBep2_I[i*incb].real || cBep1_I[i*incb].imag!=cBep2_I[i*incb].imag) {
      printf("Error! Difference between vcTanI and vmcTanI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vcTanI test/example program\n\n");
  printf("           Argument                           vcTan\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
