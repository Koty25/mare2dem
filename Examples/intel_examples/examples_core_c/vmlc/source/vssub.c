...
int main()
{
  ...
  vsSubI(vec_len,fA_I,inca,fA_I,inca,fBha0_I,incb);

  vmsSub(vec_len,fA,fA,fBep1,VML_EP);
  vmsSubI(vec_len,fA_I,inca,fA_I,inca,fBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vsSub(vec_len,fA,fA,fBep2);
  vsSubI(vec_len,fA_I,inca,fA_I,inca,fBep2_I,incb);

  vmsSub(vec_len,fA,fA,fBla1,VML_LA);
  vmsSubI(vec_len,fA_I,inca,fA_I,inca,fBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vsSub(vec_len,fA,fA,fBla2);
  vsSubI(vec_len,fA_I,inca,fA_I,inca,fBla2_I,incb);

  vmsSub(vec_len,fA,fA,fBha1,VML_HA);
  vmsSubI(vec_len,fA_I,inca,fA_I,inca,fBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vsSub(vec_len,fA,fA,fBha2);
  vsSubI(vec_len,fA_I,inca,fA_I,inca,fBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(fBha0_I[i*incb]!=fBha1_I[i*incb] || fBha1_I[i*incb]!=fBha2_I[i*incb]) {
      printf("Error! Difference between vsSubI and vmsSubI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(fBla1_I[i*incb]!=fBla2_I[i*incb]) {
      printf("Error! Difference between vsSubI and vmsSubI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(fBep1_I[i*incb]!=fBep2_I[i*incb]) {
      printf("Error! Difference between vsSubI and vmsSubI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vsSubI test/example program\n\n");
  printf("                     Arguments                               vsSub\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
