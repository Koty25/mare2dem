...
int main()
{
  ...
  vzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBha0_I,incb);

  vmzMulByConj(vec_len,zA,zA,zBep1,VML_EP);
  vmzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBep1_I,incb,VML_EP);

  vmlSetMode(VML_EP);
  vzMulByConj(vec_len,zA,zA,zBep2);
  vzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBep2_I,incb);

  vmzMulByConj(vec_len,zA,zA,zBla1,VML_LA);
  vmzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBla1_I,incb,VML_LA);

  vmlSetMode(VML_LA);
  vzMulByConj(vec_len,zA,zA,zBla2);
  vzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBla2_I,incb);

  vmzMulByConj(vec_len,zA,zA,zBha1,VML_HA);
  vmzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBha1_I,incb,VML_HA);

  vmlSetMode(VML_HA);
  vzMulByConj(vec_len,zA,zA,zBha2);
  vzMulByConjI(vec_len,zA_I,inca,zA_I,inca,zBha2_I,incb);
  ...
  for(i=0;i<10;i++) {
    ...
    if(zBha0_I[i*incb].real!=zBha1_I[i*incb].real || zBha0_I[i*incb].imag!=zBha1_I[i*incb].imag ||
      ...
      printf("Error! Difference between vzMulByConjI and vmzMulByConjI in VML_HA mode detected.\n");
      return 1;
    }
    ...
    if(zBla1_I[i*incb].real!=zBla2_I[i*incb].real || zBla1_I[i*incb].imag!=zBla2_I[i*incb].imag) {
      printf("Error! Difference between vzMulByConjI and vmzMulByConjI in VML_LA mode detected.\n");
      return 1;
    }
    ...
    if(zBep1_I[i*incb].real!=zBep2_I[i*incb].real || zBep1_I[i*incb].imag!=zBep2_I[i*incb].imag) {
      printf("Error! Difference between vzMulByConjI and vmzMulByConjI in VML_EP mode detected.\n");
      return 1;
    }
  }
  ...
  printf("\n");
  printf("vzMulByConjI test/example program\n\n");
  printf("           Arguments                           vzMulByConj\n");
  printf("===============================================================================\n");
  for(i=0;i<10;i++) {
