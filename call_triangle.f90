...
!
    ...
    out%y = in%y(1:out%nnod)
    allocate (out%z(out%nnod))  
    out%z = in%z(1:out%nnod)    
    
    out%nele = in%nele
    allocate( out%attr(in%nele), out%emap(3,in%nele) ) !
    
    out%attr = in%attr(1:in%nele)
    out%emap = in%emap(:,1:in%nele)
    
    allocate (out%neighborlist(3,in%nele) )

    out%neighborlist= in%neighborlist(:,1:out%nele)

    out%numberofpointattributes = in%numberofpointattributes
    allocate(out%pointattributelist(out%numberofpointattributes*out%nnod))
    out%pointattributelist = in%pointattributelist(1:out%numberofpointattributes*out%nnod)

    allocate(out%pointmarkerlist(out%nnod))
    out%pointmarkerlist = in%pointmarkerlist(1:out%nnod) 
...
!    out%area = in%area
    ...
    allocate( out%segmentlist (2*out%numberofsegments) )
    out%segmentlist = in%segmentlist(1:2*out%numberofsegments)
    allocate( out%segmentmarkerlist (out%numberofsegments) )
    out%segmentmarkerlist = in%segmentmarkerlist(1:out%numberofsegments)    
...
!   
 ...
 !   
    ...
    if  ( model%numberofpointattributes > 0 ) then
        write(cnpa,*) model%numberofpointattributes
        nfmt = '(1(i0,1x),2(E22.14,1x),'//trim(adjustl(cnpa))//'(E22.14,1x),'// 'i0)'
    else
        nfmt = '(1(i0,1x),2(E22.14,1x),'// 'i0)'
    endif
