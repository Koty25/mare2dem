...
! - ScaLAPACK is used for fast parallel solution of the dense matrix linear algebra operations.
! - or on small machines the multithreaded LAPACK and BLAS libraries are used.
...
!    
    integer :: nMKL_numThreads = 4 ! adjust this for the number of cores on a given cluster node
...
!             
    ...
    real(RealPrec), parameter                   :: stepSizeTol    = 2.**(1-maxNumStepCuts) ! Stop if targetRMS obtained and   
                                                                                           ! stepSize is smaller than this    
    integer                                     :: numStepCuts    = 0
    
    integer                                     :: nParallelProc
    integer, parameter                          :: MinProcScaLAPACK = 4 
...
! 
    ...
    integer                                     :: desc_wjtwj( 9) 
    real(RealPrec), dimension(:,:),allocatable  :: wjtwj_loc
...
!     
    ...
    allocate ( pm_b(nParams), pm_c(nParams), pm_test(nParams), dm_b(nd),  dm_c(nd), stat=ierr)    
    if (ierr .ne. 0) then
...
#endif
    ! if PC or only a few processors, store matrix globally:
    allocate ( amat(nParams,nParams), stat=ierr)    
    if (ierr .ne. 0) then
        write(cStr,'(a,i8)')  'Out of memory in Occam.  Too many free parameters: ', nParams 
        call printOccamLog(cStr)  
        stop 
...
!==================================================================================================================================! 
    subroutine computeOccamIteration(lSaveJacobian,lSaveSensitivity) 
!
...
!================================================================================================================== computeJacobian
!==================================================================================================================================! 
    subroutine computeJacobian(lSaveJacobian,lSaveSensitivity) 
!
...
!    
    if (lSaveJacobian) call writeJacobian(currentIteration) 
    
!
! Save sensitivity if requested:
!
    if (lSaveSensitivity) call writeSensitivity(currentIteration)  
...
! 
    ...
    ! Form (WJ)^T W \hat(d)
    !
    allocate(wjtwd(nParams), stat = ierr )   
    if (ierr .ne. 0) then
    ...
    if (nParallelProc < MinProcScaLAPACK) then
#endif
        ! If on a PC, or only using a few processors, then store full matrix on root node:
        allocate(wjtwj(nParams,nParams), stat = ierr )   
        if (ierr .ne. 0) then
            write(cStr,'(a,1x,i8)') 'Out of memory in Occam.  Too many free parameters: ', nParams 
            call printOccamLog(cStr)
            stop 
        ...
        wjtwj = 0
        ! dense matrix multiplication using LAPACK:
...
! Local variables:
!    
    real(RealPrec)  :: minv, maxv, mu, t0, t1, t2,  chi2 
    integer         :: istat 
    character(128)  :: cStr
...
!
    ...
    if (nParallelProc < MinProcScaLAPACK) then
        ...
        call solveLinearSystem_scalapack(istat,mu) 
    endif
#endif
...
!====================================================================================================================== constructRHS
!==================================================================================================================================! 
    subroutine constructRHS(mu)
    
    real(RealPrec), intent(in)  :: mu
...
! Local variables:
!    
    real(RealPrec)  :: w_i, w_j, w2, w_mgs
    integer         :: i,j, irow, istart,iend,i1,i2, ipen
...
!-----------------------------------------------------------------------------------------------------------------------------------  
    ...
    call  mkl_set_num_threads(min(nParallelProc,nMKL_numThreads))
#endif
...
!       
    use mare2dem_scalapack
    
    implicit none

    integer  :: desc_wj( descriptor_len ) 
    real(RealPrec), dimension(:,:),allocatable  :: wj_loc 
...
!
! Allocate the distributed wj matrix:
!
    call allocateDistributedMatrix(size(wj,1),size(wj,2),desc_wj,wj_loc)
 
! 
! Distribute matrix WJ in block cyclic form to grid:
!
    call scalapack_scatterMatrix(wj,wj_loc,desc_wj)

!
! Deallocate WJ on root node now that it has been distributed to grid:
! 
    if (myid == 0) deallocate(wj)    
 
!
! Create storage for distributed wjtwj matrix:
! 
    call allocateDistributedMatrix(desc_wj(4),desc_wj(4),desc_wjtwj,wjtwj_loc)    
 
!
! Parallel matrix multiply:
!        
    call pdsyrk('u','t',desc_wj(4),desc_wj(3),1.d0,wj_loc,1,1,desc_wj,0.d0,wjtwj_loc,1,1,desc_wjtwj) 
 
!
! Deallocate wj_loc since we are done with it:
!
    if (allocated(wj_loc))    deallocate(wj_loc)
    
! At this point each worker has a wjtwj_loc so we are all done. They will sit in memory
! until called upon...
...
!--------------------------------------------------------------------------------------------------
!--------------------------------------------------------------------------------------------------  
    subroutine mpi_worker_ScaLapackMS(istat,mu)
!
...
!
    ...
    integer :: desc_amat( descriptor_len ), desc_pm_test( descriptor_len ) 
    real(RealPrec), dimension(:,:),allocatable  :: amat_loc 
    real(RealPrec), dimension(:),allocatable    :: pm_test_loc   
 
   
    real(RealPrec)  :: w_i, w_j, w2, w_mgs
    integer         :: i,j, irow, istart,iend,i1,i2,rsrc,csrc,i_loc,j_loc
...
!
    if ( (myrow == 0) .AND. (mycol == 0) ) then
        call dgebs2d(ictxt, 'All', 'i-ring', 1, 1, mu, 1)
    else
        call dgebr2d(ictxt, 'All', 'i-ring', 1, 1, mu, 1, 0, 0)
    endif 
    
!
! Allocate the storage for distributed amat matrix and rhs vector pm_test:
!
    call allocateDistributedMatrix(nParams,nParams,desc_amat,amat_loc)

    call allocateDistributedVector(size(pm_test,1),desc_pm_test,pm_test_loc)
 
! 
! Distribute RHS vector to processor grid:
!
    call scalapack_scatterVector(pm_test,pm_test_loc,desc_pm_test)
    
        
    if (lMGS) then  ! broadcast pm to all processes for use in MGS weights
        if (myid>0)  allocate(pm(nParams))
        call mpi_bcast(pm, nParams, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)     
    endif
    
!
! Assemble amat_loc entries for this subarray:
!    
  ! First insert penalty terms for del^T del:       
    do irow = 1,pen%nrows 
        ...
        do i1 = istart,iend
            do i2 = istart,iend
                ...
                ! Is entry i,j in the local subarray?
                call infog2l(i,j,desc_amat,nprow,npcol,myrow,mycol, i_loc, j_loc, rsrc, csrc )
                ...
                if ((rsrc == myrow) .and. (csrc == mycol) ) then
                    ...
                    amat_loc(i_loc,j_loc) = amat_loc(i_loc,j_loc) + w_i*w_j*w_mgs 
   ...
   ! Add on the penalty terms for p^T p, where p is diagonal:
    ...
    do i = 1,nParams
        if (prewts(i) /= 0) then ! prewts is usually zero, unless prejudice requested
            call infog2l(i,i,desc_amat,nprow,npcol,myrow,mycol, i_loc, j_loc, rsrc, csrc )
            if ((rsrc == myrow) .and. (csrc == mycol) ) then
                amat_loc(i_loc,j_loc) = amat_loc(i_loc,j_loc) + prewts(i)**2
            endif
        endif
    end do
   ...
   do irow = 1,nDiff
       ...
       w2 = preDiffwts(irow)**2
       call infog2l(i,j,desc_amat,nprow,npcol,myrow,mycol, i_loc, j_loc, rsrc, csrc )
       if ((rsrc == myrow) .and. (csrc == mycol) )  amat_loc(i_loc,j_loc) = amat_loc(i_loc,j_loc)  - w2
       call infog2l(j,i,desc_amat,nprow,npcol,myrow,mycol, i_loc, j_loc, rsrc, csrc )
       if ((rsrc == myrow) .and. (csrc == mycol) )  amat_loc(i_loc,j_loc) = amat_loc(i_loc,j_loc)  - w2
       call infog2l(i,i,desc_amat,nprow,npcol,myrow,mycol, i_loc, j_loc, rsrc, csrc )
       if ((rsrc == myrow) .and. (csrc == mycol) )  amat_loc(i_loc,j_loc) = amat_loc(i_loc,j_loc)  + w2
       call infog2l(j,j,desc_amat,nprow,npcol,myrow,mycol, i_loc, j_loc, rsrc, csrc )
       if ((rsrc == myrow) .and. (csrc == mycol) )  amat_loc(i_loc,j_loc) = amat_loc(i_loc,j_loc)  + w2
   enddo    
 
    ! Add on the WJ^T WJ   
    amat_loc = mu*amat_loc + wjtwj_loc    
    
!
! Parallel matrix solver:
!       
    !write(*,*) 'calling pdpotrf...'
    call pdpotrf( 'u',  desc_amat(3), amat_loc, 1, 1, desc_amat, istat )     
    if (istat /= 0) then
        write(*,*) 'Error calling pdpotrf! info: ',istat
        !stop
    endif  

    !write(*,*) 'calling pdpotrs...'
    call pdpotrs( 'u',  desc_amat(3), 1, amat_loc, 1, 1, desc_amat, pm_test_loc, 1, 1, desc_pm_test,  istat )
    if (istat /= 0) then
        write(*,*) 'Error calling pdpotrs! info: ',istat
        !stop
    endif  
 
!
! Deallocate amat_loc since we are done with it:   ! KWK debug we will save this in future Occam...
!
...
!   
    call scalapack_gatherVector(pm_test_loc,desc_pm_test,pm_test)
 
!
! Deallocate local pm_test_loc arrays
!
    if (allocated(pm_test_loc)) deallocate(pm_test_loc)
    
    if ((lMGS).and.(myid>0)) deallocate(pm)    
...
!            of the exponential. Updated November 2014.
!  
!---------------------------------------------------------------------
...
!        
      ...
      real(8) :: a, b, c, q, x ! using double no matter what here to avoid exp issues
      ...
      ! Convert input to double variables:
        ...
        if (lbind) then
            ...
            case ('exponential')  
             ...
             if ( x.le.0) then
                 x =  ( a*exp( x) + b )  / ( exp( x) + 1d0 )
             else
                 x =  ( b*exp(-x) + a )  / ( exp(-x) + 1d0 )
             endif
            ...
            case ('bandpass')
             ...
             if (x.le.0) then
                ...
                q =  log( (1d0+exp(c*(b-x))) / (1d0+exp(c*(a-x))) )
             endif       
...
!
      ...
      ! Note that the returned value explodes if m = a or m = b, so input models
      ! need to be moved a tad numerically from these values.
      ...
      ! Convert input to double variables:
        ...
        if (lbind) then
            ...
            case ('bandpass')
                ...
                if (m.le.0) then
                    q = log( (exp(c*(m-b))-1d0) / (1d0-exp(c*(m-a))) )  
                else
            ...
            if (lBoundMe(j)) then
                ...
                case ('bandpass')
                     ...
                     if (pdp.le.0)  then
                        p = exp( c*(pdp-b))
                        q = exp( c*(pdp-a))
                        dmdx = p*(1d0-exp(-bandPassFactor))/ ( (1d0+p)*(1d0+q) )
                     elseif (pdp.gt.0) then
                        p = exp(-c*(pdp-b)) 
                        q = exp(-c*(pdp-a)) 
                        dmdx = q*(1d0-exp(-bandPassFactor)) / ( (1d0+p)*(1d0+q) )
                     endif
                      wj(:,j) = wj(:,j) * dmdx                
